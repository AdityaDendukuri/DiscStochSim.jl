...
# ╔═╡ 3737bf50-0d99-40bc-b39d-133ac4195736
begin
	using Catalyst, CommonSolve, Distributions
...
md"""
# ELSE, from the perspective of SSA and FSP

A reaction system with **two basins**. The trajectory sits in one basin for a very
long time, then rarely hops to the other. The question this notebook answers three
ways: **when does it hop, and where does it cross?**

- **SSA** — simulate *every* reaction until the hop. Correct, but it burns through
  *millions* of fast in-basin reactions before the one rare reaction that escapes.
  Cost = number of reactions.
- **FSP** — never simulate. Truncate the state space and solve `dp/dt = A·p` for the
  whole probability distribution, out to some horizon `tf`. Cost = evolving that
  distribution in time.
- **ELSE** — a trajectory method like SSA, but it *refuses* to simulate the millions
  of in-basin reactions. It takes the basin as a block `J`, samples the exit state
  from the fundamental matrix `Z`, and then samples the escape time conditional on
  that state. Cost is set by the basin size, **not** by how rare the hop is.

ELSE deletes exactly the part SSA wastes time on. It reuses the **same generator**
FSP builds (`A_JJ`), but instead of stepping it through every reaction it treats
the whole basin as one transient block. The engine is the fundamental matrix
`Z = -inv(A_JJ)`.

Convention: Fornace's row matrix `R = A_JJ'`; `build_generator` here gives the
column generator `A_JJ` (`dp/dt = A_JJ p`), so our `Z = -inv(A_JJ)` is his transpose.
Column convention throughout.
"""

# ╔═╡ 6c996752-ad54-47e7-a5cc-eeae36a336f0
gr()

# ╔═╡ e0000005-0000-4000-8000-000000000005
md"""
## 1. Model — toggle switch

Mutual Hill repression, degradation at rate `d`:

```math
\varnothing \xrightarrow{h(Y)} X,\quad
\varnothing \xrightarrow{h(X)} Y,\quad
X \xrightarrow{d}\varnothing,\quad Y\xrightarrow{d}\varnothing,\quad
h(z)=\frac{b\,K^{n}}{K^{n}+z^{n}}.
```

Bistable for `n ≥ 2`: an `X`-high well and a `Y`-high well split by the diagonal
`x = y`. Crossing it is the hop.
"""

# ╔═╡ fcd1e709-fdeb-4cd2-b15a-2458f9ca0f1b
...
# ╔═╡ bb818ded-7cd3-45d8-913d-00a8f51ed520
model = DiscreteStochasticSystem(rn)

# ╔═╡ f9a42a7e-5fa3-432e-81e4-d1b5a76dc776
hillr(z, v, K, n) = v * K^n / (K^n + z^n)

# ╔═╡ e0000009-0000-4000-8000-000000000009
md"""
Basin `J` = the `X`-high well = states with `x ≥ y`. `basin` enumerates it;
`xhigh_mode` is the well bottom, the start state `p0 = δ_s0`.
"""

# ╔═╡ a35865ec-c939-413a-b0e1-3d6e00b56579
basin(G) = begin
...
# ╔═╡ 109860b0-683a-421c-8893-4392aa0d1f0e
...
# ╔═╡ e000000c-0000-4000-8000-00000000000c
md"""
## 2. The SSA baseline

Every reaction, to the first crossing `y > x`. Returns hop time, event count, and
the separatrix state left from. This is the reference ELSE must match — and the
event count is the work ELSE replaces with one solve.
"""

# ╔═╡ 3cb6d234-c304-4168-b9f1-0b950607d4b4
...
# ╔═╡ e000000e-0000-4000-8000-00000000000e
md"""
## 3. ELSE, object by object

### 3a. Subnetwork generator `A_JJ`

Same generator FSP builds, restricted to the basin `J`, with every edge leaving `J`
absorbed (the transient subnetwork, Fornace Fig. 1). `build_generator(...;
absorbing=true)` returns `A_JJ` and the per-state escape rate `Δ_j = Σ_{y∉J}
rate(j→y)`. Column sums:

```math
\mathbf{1}^\top \mathbf{A}_{JJ} = -\,\boldsymbol{\Delta}^\top .
```

Every state escapes, so `A_JJ` is negative-definite (invertible).
"""

# ╔═╡ e000000f-0000-4000-8000-00000000000f
...
md"""
### 3b. Fundamental matrix `Z` — Fornace **Eq. 8**

```math
Z = -\mathbf{A}_{JJ}^{-1} = \int_0^\infty e^{t\mathbf{A}_{JJ}}\,dt \ \ge 0 .
```

`Z_ji` = expected time in `j` before escape, started at `i`. It is the time-integral
of the FSP semigroup `exp(t·A_JJ)` — the operator FSP steps forward, here integrated
all the way to escape. We need only `Z p0 = -inv(A_JJ) p0` — one solve.
"""

# ╔═╡ e0000011-0000-4000-8000-000000000011
occupation(sub) = -(sub.A \ sub.p0)      # Z p0 = -inv(A_JJ) p0   (one solve)

# ╔═╡ e0000012-0000-4000-8000-000000000012
md"""
### 3c. Mean escape time — Fornace **Eq. 15**

```math
\mathbb{E}[\tau] = \mathbf{1}^\top Z\,\mathbf{p}_0 .
```
This one number is what the ELSE *algorithm* reports per subnetwork.
"""

# ╔═╡ e0000013-0000-4000-8000-000000000013
mean_escape(sub) = sum(occupation(sub))       # 1' Z p0

# ╔═╡ e0000014-0000-4000-8000-000000000014
md"""
### 3d. Variance / moments — Fornace **Eq. 14**

`τ` is phase-type`(p0, A_JJ)`; each moment is a power of `Z`:

```math
\mathbb{E}[\tau^m] = m!\,\mathbf{1}^\top Z^{m}\mathbf{p}_0,
\qquad
\mathrm{Var}(\tau) = 2\,\mathbf{1}^\top Z^{2}\mathbf{p}_0 - (\mathbf{1}^\top Z\mathbf{p}_0)^2 .
```
"""

# ╔═╡ e0000015-0000-4000-8000-000000000015
function escape_var(sub)
    z1 = occupation(sub)                # Z p0
    z2 = sub.A \ (sub.A \ sub.p0)       # Z² p0 = inv(A)(inv(A) p0)
    2*sum(z2) - sum(z1)^2
end

# ╔═╡ e0000016-0000-4000-8000-000000000016
md"""
### 3e. Sample the escape state first — Fornace **Eq. 10**

Escape from state `j` with probability (escape rate) × (occupation):

```math
\pi_{\mathrm{exit}}(j) \propto \Delta_j\,(Z\mathbf{p}_0)_j .
```
These probabilities are nonzero only on the separatrix `x = y`. ELSE samples this
state **before** sampling the time.
"""

# ╔═╡ e0000017-0000-4000-8000-000000000017
function exit_states(sub)
    w = sub.Δ .* occupation(sub)        # Δ ∘ (Z p0)
    w ./ sum(w)
end

# ╔═╡ e0000018-0000-4000-8000-000000000018
md"""
### 3f. Full escape law — theory Eq. (†)

```math
\Gamma_J(t)=\mathbf{1}^\top e^{t\mathbf{A}_{JJ}}\mathbf{p}_0
\quad(\text{survival}),
\qquad
f(t)=\boldsymbol{\Delta}^\top e^{t\mathbf{A}_{JJ}}\mathbf{p}_0=\Phi_{\mathrm{out}}(t)
\quad(\text{density}).
```
`f = Φ_out` is the boundary outflux — what FSP calls truncation error is ELSE's
answer. And `∫ Γ_J dt = 1' Z p0 = E[τ]`. Evaluate `exp(t A_JJ) p0` with `expv`.
"""

# ╔═╡ e0000019-0000-4000-8000-000000000019
begin
	survival(sub, t)       = sum(expv(t, sub.A, sub.p0))        # Γ_J(t)
	escape_density(sub, t) = dot(sub.Δ, expv(t, sub.A, sub.p0)) # f(t) = Φ_out(t)
end

# ╔═╡ e000001a-0000-4000-8000-00000000001a
md"""
## 4. State-first stochastic ELSE

Write the eigendecomposition once as
...
First sample the boundary state \$B=b\$ from

\$\$
\Pr(B=b\mid X_0=a)=\Delta_b Z_{ba}.
\$\$

Then Equation 32 gives the survival probability conditioned on that state:
...
\left[
U\operatorname{diag}
\left(e^{-\lambda T}/\lambda\right)U^{-1}
\right]_{ba}
}{
Z_{ba}
}.
...
The code draws \$u\sim\operatorname{Uniform}(0,1)\$ and solves this scalar equation
by bisection. The eigendecomposition is shared by every sample; bisection evaluates
only scalar exponentials.
"""

# ╔═╡ e000001b-0000-4000-8000-00000000001b
function else_sampler(sub)
    F = eigen(Matrix(sub.A))
    factorization = lu(sub.A)
    U = ComplexF64.(F.vectors)
    U_inverse = inv(U)
    modes = U_inverse * sub.p0
    occ = -(factorization \ sub.p0)              # Z p₀
    occ2 = factorization \ (factorization \ sub.p0) # Z² p₀

    exit_probability = max.(sub.Δ .* occ, 0.0)
...
# ╔═╡ e000001c-0000-4000-8000-00000000001c
function sample_index(probability, rng)
    target = rand(rng)
    total = 0.0
...
# ╔═╡ e000001d-0000-4000-8000-00000000001d
md"""
`else_switch` is one exact state-first hop:

1. sample the escape state using \$\Delta_b Z_{ba}\$;
2. sample the escape time conditioned on that state;
3. cross the separatrix in one step.

One call replaces the entire SSA `while` loop.
"""

# ╔═╡ a0f7fda4-98cd-44c1-899c-c229780fb013
function conditional_survival(w, exit_state, time)
    coefficients =
        w.U[exit_state, :] .* w.modes ./ (-w.eigenvalues)

    numerator =
        sum(coefficients .* exp.(w.eigenvalues .* time))

    normalization = sum(coefficients)              # Z_ba in the same eigenbasis
    survival = real(numerator / normalization)
    clamp(survival, 0.0, 1.0)
end

function conditional_escape_time(w, exit_state, rng)
    target = max(rand(rng), eps(Float64))
    lo = 0.0

    hi = real(w.occ2[exit_state] / w.occ[exit_state])
    hi = hi > 0 ? hi : w.mfpt

    while conditional_survival(w, exit_state, hi) > target
        hi *= 2
    end

    for _ in 1:60
        mid = (lo + hi) / 2
        if conditional_survival(w, exit_state, mid) > target
            lo = mid
        else
            hi = mid
        end
...
function else_switch(w, rng)
    exit_state = sample_index(w.exit_probability, rng)
    waiting_time = conditional_escape_time(w, exit_state, rng)
    crossing_x = w.sp.states[exit_state][1]

    (t=waiting_time, x=crossing_x)
end

# ╔═╡ e000001f-0000-4000-8000-00000000001f
md"""
Same statistic, two engines. ELSE preserves the joint distribution of switch time
and crossing state; SSA pays thousands of events per sample.
"""

# ╔═╡ 6d052650-59e6-4251-9976-3b8155eb5624
b, K, d, G, n = 20.0, 10.0, 1.0, 45, 3.0

# ╔═╡ b5886c4e-9ceb-432f-8484-65a42f929991
M = 5000

# ╔═╡ e0000040-0000-4000-8000-000000000040
sub = subnetwork(G, b, K, n, d)                      # the ELSE subnetwork (sparse A_JJ)

# ╔═╡ 9a644d36-3da5-472f-a76a-24b8c6fd9499
w = else_sampler(sub)                                # shared Z data + eigendecomposition

# ╔═╡ e0000022-0000-4000-8000-000000000022
md"ELSE summaries + shared subnetwork size:"

# ╔═╡ e0000023-0000-4000-8000-000000000023
(sub_size = length(w.sp),
 mean_escape_time = w.mfpt,
 std_escape_time  = sqrt(escape_var(w)))

# ╔═╡ be76c962-3cbe-40bf-8c79-d438c56f888e
g = let rng = MersenneTwister(1)
	[gillespie_switch(b, K, n, d, rng) for _ in 1:M]
end

# ╔═╡ 2f8625a8-8d20-4871-ab92-de7fc8edd5fa
e = let rng = MersenneTwister(2)
	[else_switch(w, rng) for _ in 1:M]
end

# ╔═╡ 8a0f40a6-f9ec-4163-97b4-96f24c40003a
(gillespie_mean = mean(x->x.t, g), else_mean = mean(x->x.t, e),
 gillespie_events_per_switch = round(Int, mean(x->x.events, g)),
 else_cost = "state first + $M conditional time draws")

# ╔═╡ 95af1af3-07a6-4892-873b-25845ea19332
let
	g_t = [x.t for x in g];  e_t = [x.t for x in e]
	ts  = range(0, quantile(g_t, 0.995); length=200)
	cdf(v) = [count(<=(t), v)/length(v) for t in ts]
	plot(ts, cdf(g_t); lw=3, color=1, label="SSA (every reaction)",
	     xlabel="first-switch time", ylabel="CDF",
	     title="ELSE switch times = SSA", legend=:bottomright, size=(720,430))
	plot!(ts, cdf(e_t); lw=2, ls=:dash, color=3, label="ELSE (one draw / switch)")
end

# ╔═╡ f1e4c975-c692-4778-91eb-96fa7873885a
let
	g_x = [x.x for x in g];  e_x = [x.x for x in e]
	xr  = minimum(min(minimum(g_x),minimum(e_x))):maximum(max(maximum(g_x),maximum(e_x)))
	hist(v) = [count(==(x), v)/length(v) for x in xr]
	bar(collect(xr), hist(g_x); bar_width=0.6, color=1, alpha=0.5, label="SSA",
	    xlabel="crossing coordinate x", ylabel="exit probability",
	    title="ELSE exit state (Eq. 10) = SSA", size=(720,430))
	scatter!(collect(xr), hist(e_x); marker=:circle, ms=5, color=3, label="ELSE")
end

# ╔═╡ a1000001-1000-4000-8000-000000000001
md"""
## 5. One million trajectories as population counts

The toggle is symmetric. After crossing from \$X=Y=x\$ into the \$Y\$-high basin
at \$(x,x+1)\$, swap the species labels. The canonical entry state is then
\$(x+1,x)\$ in the same \$X\$-high subnetwork. We can therefore reuse one
factorization and one eigendecomposition for every switch.

For \$N_a\$ trajectories entering at state \$a\$, sample all exits together:

\$\$
(N_{1a},\ldots,N_{Ka})
\sim
\operatorname{Multinomial}
\left(N_a,\{\Delta_bZ_{ba}\}_{b=1}^{K}\right).
\$\$

The code stores counts keyed by entry state. It does not allocate one million
trajectory objects.
"""

# ╔═╡ a1000002-1000-4000-8000-000000000002
function entry_law(w, entry_state)
    p0 = zeros(length(w.sp))
    p0[entry_state] = 1.0

    occupation = -(w.factorization \ p0)
    exit_probability = max.(w.Δ .* occupation, 0.0)
    exit_probability ./= sum(exit_probability)

    (
        modes=w.U_inverse[:, entry_state],
        exit_probability=exit_probability,
    )
end

# ╔═╡ a1000003-1000-4000-8000-000000000003
function conditional_survival(w, law, exit_state, time)
    coefficients =
        w.U[exit_state, :] .* law.modes ./ (-w.eigenvalues)

    numerator =
        sum(coefficients .* exp.(w.eigenvalues .* time))

    normalization = sum(coefficients)
    clamp(real(numerator / normalization), 0.0, 1.0)
end

# ╔═╡ a1000004-1000-4000-8000-000000000004
function batched_switch_sequences(w, number_of_trajectories, number_of_switches, rng)
    start_state = findfirst(>(0.0), w.p0)
    grid_limit = maximum(state[1] for state in w.sp.states)

    groups = Dict(start_state => number_of_trajectories)
    laws = Dict{Int, Any}()
    history = NamedTuple[]
    ...
    for switch in 1:number_of_switches
        ...
        exit_counts = zeros(Int, length(w.sp))
        outside_grid = 0

        for (entry_state, group_size) in groups
            law = get!(laws, entry_state) do
                entry_law(w, entry_state)
            end

            counts = rand(
                rng,
                Multinomial(group_size, law.exit_probability),
            )

            for exit_state in findall(>(0), counts)
                count = counts[exit_state]
                transitions[(entry_state, exit_state)] = count
                exit_counts[exit_state] += count

                state = w.sp.states[exit_state]
                crosses_separatrix =
                    state[1] == state[2] && state[1] < grid_limit

                if crosses_separatrix
                    # (x,x) → (x,x+1), then swap labels → (x+1,x)
                    next_state =
                        w.sp.index[CartesianIndex(state[1] + 1, state[1])]
                    next_groups[next_state] =
                        get(next_groups, next_state, 0) + count
                else
                    outside_grid += count
                end
            end
        ...
        push!(
            ...
            (
                switch=switch,
                groups=copy(next_groups),
                transitions=transitions,
                exit_counts=exit_counts,
                active=sum(values(next_groups)),
    ...
    (history=history, entry_laws=laws)
end

# ╔═╡ a1000005-1000-4000-8000-000000000005
begin
    number_of_batched_trajectories = 1_000_000
    number_of_basin_switches = 4

    batched = batched_switch_sequences(
        w,
        number_of_batched_trajectories,
        number_of_basin_switches,
        MersenneTwister(2026),
    )
end

# ╔═╡ a1000006-1000-4000-8000-000000000006
[
    (
        switch=result.switch,
        basin=isodd(result.switch) ? "Y-high" : "X-high",
        trajectories=result.active,
        active_entry_states=length(result.groups),
        grid_overflow=result.outside_grid,
    )
    for result in batched.history
]

# ╔═╡ a1000007-1000-4000-8000-000000000007
let
    first_switch = batched.history[1]
    grid_limit = maximum(state[1] for state in w.sp.states)
    crossing_x = 0:(grid_limit - 1)

    crossing_counts = [
        sum(
            first_switch.exit_counts[j]
            for j in eachindex(first_switch.exit_counts)
            if w.sp.states[j] == CartesianIndex(x, x)
        )
        for x in crossing_x
    ]
    ...
    bar(
        crossing_x,
        crossing_counts ./ sum(crossing_counts);
        color=:steelblue,
        legend=false,
        xlabel="separatrix coordinate x",
        ylabel="fraction entering the Y-high basin",
        title="One million batched ELSE trajectories",
        size=(720, 430),
    )
end

# ╔═╡ a1000008-1000-4000-8000-000000000008
md"""
### Deferred time reconstruction

After the discrete switches have been sampled, transitions sharing \$(a,b)\$ also
share the same conditional CDF. Evaluate Equation 32 on a time grid and make one
more multinomial draw over the time bins. This recovers the million-trajectory
time histogram without one million inverse-CDF solves.
"""

# ╔═╡ a1000009-1000-4000-8000-000000000009
function batched_time_histogram(w, transitions, entry_laws, time_edges, rng)
    interval_counts = zeros(Int, length(time_edges) - 1)
    tail_count = 0

    for ((entry_state, exit_state), group_size) in transitions
        law = entry_laws[entry_state]
        survival = [
            conditional_survival(w, law, exit_state, time)
            for time in time_edges
        ]

        probability = [
            max(survival[i] - survival[i + 1], 0.0)
            for i in 1:(length(time_edges) - 1)
        ]
        push!(probability, max(survival[end], 0.0))
        probability ./= sum(probability)

        counts = rand(rng, Multinomial(group_size, probability))
        interval_counts .+= counts[1:end-1]
        tail_count += counts[end]
    end

    (interval_counts=interval_counts, tail_count=tail_count)
end

# ╔═╡ a100000a-1000-4000-8000-00000000000a
begin
    batched_time_edges =
        range(0.0, 8 * w.mfpt; length=250)

    first_switch_times = batched_time_histogram(
        w,
        batched.history[1].transitions,
        batched.entry_laws,
        batched_time_edges,
        MersenneTwister(2027),
    )
end

# ╔═╡ a100000b-1000-4000-8000-00000000000b
let
    batched_cdf =
        cumsum(first_switch_times.interval_counts) ./
        number_of_batched_trajectories

    ssa_times = [sample.t for sample in g]
    ssa_cdf = [
        count(<=(time), ssa_times) / length(ssa_times)
        for time in batched_time_edges[2:end]
    ]

    plot(
        batched_time_edges[2:end],
        batched_cdf;
        lw=3,
        color=:steelblue,
        label="1,000,000 batched ELSE",
        xlabel="first-switch time",
        ylabel="CDF",
        title="Deferred conditional-time reconstruction",
        legend=:bottomright,
        size=(720, 430),
    )
    plot!(
        ...
        ls=:dash,
        color=:black,
        label="$(length(g)) SSA trajectories",
    )
end

# ╔═╡ a100000c-1000-4000-8000-00000000000c
(
    discrete_trajectories=number_of_batched_trajectories,
    reached_opposite_basin=batched.history[1].active,
    unresolved_grid_exits=batched.history[1].outside_grid,
    time_histogram_tail=first_switch_times.tail_count,
)

# ╔═╡ e0000050-0000-4000-8000-000000000050
md"""
## 6. Making it scale — the resolvent (Laplace transform)

The eigendecomposition in §4 is **dense** and won't scale. The escape law lives in
the **resolvent** `(sI − A_JJ)⁻¹`, the Laplace transform of the semigroup:

```math
(sI - \mathbf{A}_{JJ})^{-1}\,\mathbf{p}_0 = \int_0^\infty e^{-st}\,e^{t\mathbf{A}_{JJ}}\mathbf{p}_0\,dt .
```

`s = 0` gives `Z` (mean, exit). Sweeping `s` gives the whole distribution: the escape
density transform is `f̂(s) = Δᵀ(sI − A_JJ)⁻¹p₀`. We evaluate it at fixed Bromwich
nodes `s_k = a + i·kπ/T` — **one sparse solve each, shared across all times and all
samples** — then invert by the Fourier-series method. No eigendecomposition, only
sparse solves.

(We invert the *density* `f`, not the survival: started deep in the well `f` is
smooth with `f(0)=0`, so `f̂(s)` decays fast and a few hundred nodes suffice.)
"""

# ╔═╡ e0000051-0000-4000-8000-000000000051
# f̂(s) = Δᵀ (sI - A_JJ)⁻¹ p0 at fixed nodes s_k = a + i kπ/T, k=0..N — N+1 sparse solves.
function resolvent_nodes(sub; T, a, N)
    Ip0 = ComplexF64.(sub.p0)
    [ (s = a + im*(k*π/T); dot(sub.Δ, (s*I - sub.A) \ Ip0)) for k in 0:N ]
end

# ╔═╡ e0000052-0000-4000-8000-000000000052
begin
	# Fourier-series inversion: f(t) ≈ (e^{at}/T)[½Re f̂(a) + Σ Re(f̂_k e^{i kπt/T})]
	function laplace_density(fhat, t; T, a)
		N = length(fhat) - 1
		s = 0.5*real(fhat[1]) + sum(real(fhat[k+1]*cis(k*π*t/T)) for k in 1:N)
		max(0.0, (exp(a*t)/T) * s)
	end
	# tabulate density → CDF → inverse-CDF sampler
	function laplace_sampler(fhat; T, a, tmax, ngrid=4000)
		tg = range(0, tmax; length=ngrid)
		fg = [laplace_density(fhat, t; T=T, a=a) for t in tg]
		Fg = cumsum(0.5 .* (fg .+ [0.0; fg[1:end-1]]) .* step(tg)); Fg ./= Fg[end]
		(tg=collect(tg), Fg=Fg)
	end
	sample_laplace(S, rng) = S.tg[searchsortedfirst(S.Fg, rand(rng))]
end

# ╔═╡ e0000053-0000-4000-8000-000000000053
# fixed nodes, solved ONCE and reused for every time and every sample
fhat = resolvent_nodes(sub; T=500.0, a=0.015, N=400)

# ╔═╡ e0000054-0000-4000-8000-000000000054
let
	tt = range(0, 160; length=300)
	plot(tt, [escape_density(sub, t) for t in tt]; lw=4, color=1, label="exact (semigroup, expv)",
	     xlabel="escape time t", ylabel="density f(t)",
	     title="ELSE escape density via resolvent ($(length(fhat)) solves)", size=(720,430))
	plot!(tt, [laplace_density(fhat, t; T=500.0, a=0.015) for t in tt];
	      lw=2, ls=:dash, color=3, label="Laplace (resolvent, sparse solves)")
end

# ╔═╡ e0000055-0000-4000-8000-000000000055
Slap = laplace_sampler(fhat; T=500.0, a=0.015, tmax=220.0)

# ╔═╡ e0000056-0000-4000-8000-000000000056
let
	l_t = [sample_laplace(Slap, rng) for rng in (MersenneTwister(k) for k in 1:M)]
	g_t = [x.t for x in g]
	ts = range(0, quantile(g_t,0.995); length=200)
	cdf(v) = [count(<=(t),v)/length(v) for t in ts]
	plot(ts, cdf(g_t); lw=3, color=1, label="SSA", xlabel="switch time", ylabel="CDF",
	     title="Laplace-sampled switch times = SSA  (mean $(round(mean(l_t);digits=2)))",
	     legend=:bottomright, size=(720,430))
	plot!(ts, cdf(l_t); lw=2, ls=:dash, color=3, label="Laplace sampler")
end

# ╔═╡ e0000060-0000-4000-8000-000000000060
md"""
## 7. The real toggle (Tian & Burrage)

The actual model at its actual parameters: `U`/`V` mutual repression, Hill 3,
production `≈420`, degradation `≈1`, start `u0 = (85,5)`. Basin = the `U`-high well
`{U ≥ V}` on a grid large enough to **contain** the well (mode `≈ (378,27)`), so the
only escape is the `U=V` crossing. One sparse solve gives the exact mean hop time —
a quantity FSP (to `tf=30`) and SSA cannot reach.
"""

# ╔═╡ e0000061-0000-4000-8000-000000000061
rn_real = @reaction_network begin
    @species U(t) V(t)
...
# ╔═╡ e0000062-0000-4000-8000-000000000062
begin
	model_real = DiscreteStochasticSystem(rn_real)
	rates_real = [1.0, 20.0, 400.0, 100.0, 1.0, 0.1/1.1, 20.0, 400.0, 100.0, 1.0]
	GU = 680                                     # top boundary far above the well ⇒ only escape is U=V
end

# ╔═╡ e0000063-0000-4000-8000-000000000063
# ELSE on the U-high basin: build A_JJ, one solve, check escape flux is all on U=V.
real_result = let
	sp = StateSpace{CartesianIndex, Float64}()
	for u in 0:GU, v in 0:u; add_state!(sp, CartesianIndex(u, v), 0.0); end   # J = {V ≤ U ≤ GU}
	A, _, _, Δ = build_generator(sp, model_real, rates_real, 0.0; absorbing=true)
	p0 = zeros(length(sp)); p0[sp.index[CartesianIndex(85, 5)]] = 1.0
	occ = -(A \ p0)                              # one sparse solve
	wts = Δ .* occ
	onsep = [sp.states[j][1] == sp.states[j][2] for j in eachindex(wts)]
	(states = length(sp),
	 mean_hop_time = sum(occ),
	 separatrix_flux_pct = 100*sum(wts[onsep])/sum(wts),
	 crossing_U = sum(sp.states[j][1]*wts[j] for j in eachindex(wts) if onsep[j])/sum(wts[onsep]),
	 mode = sp.states[argmax(occ)])
end

# ╔═╡ e0000065-0000-4000-8000-000000000065
md"""
`mean_hop_time ≈ 3.4e14` from **one 1.7 s solve on ~232k states**. Your FSP runs to
`tf = 30` — the hop is ~**10¹³×** past that horizon. SSA would need ~10¹⁴ reaction
events per trajectory. `separatrix_flux_pct = 100` confirms the only escape is the
true `U=V` crossing (no artificial grid-edge leak); `mode` sits at the well bottom.
"""

# ╔═╡ e0000028-0000-4000-8000-000000000028
...
manifest_format = "2.0"
project_hash = "cc20e5c6e36318de3b00b26912171717ba658cab"
...
git-tree-sha1 = "7ade09a0484097ffabace37b5b7c8c0d97a42bee"
...
git-tree-sha1 = "c42c4681a7d417f62ec37ccd1c300c731f8edf2f"
...
deps = ["Adapt", "ArrayInterface", "GPUArraysCore", "GenericSchur", "LinearAlgebra", "LinearSolve", "PrecompileTools", "Printf", "SparseArrays", "libblastrampoline_jll"]
git-tree-sha1 = "e0611bac3a189d234ae90b598b1fc3c30790414a"
...
git-tree-sha1 = "98cc59ae3a70c4142c05698d7efcebca6cd0ca5e"
...
git-tree-sha1 = "4e924b19bed0ed42e42dff40537638fb497b89bb"
...
git-tree-sha1 = "3b6932f46e49d46dc68b68e4b09e076582dd9edb"
...
git-tree-sha1 = "d1630f4d1d9c53f3c56cabb00e2659671fde0dfd"
