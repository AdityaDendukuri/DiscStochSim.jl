...
# ╔═╡ 1b7e2cf0-5d70-4ac9-9fe8-07939f4d1011
begin
...
# ╔═╡ 313a89a4-096c-4496-a95d-acde2a4e8e98
md"""
# Stochastic ELSE trajectories

This notebook uses ELSE as a replacement for an SSA loop.

SSA fires one reaction at a time. ELSE instead chooses a small set of states
around the current state, integrates out every reaction that happens inside that
set, and samples the first reaction that leaves it.

The output is an **exit skeleton**:

```math
(t_0,x_0)\longrightarrow(t_1,x_1)\longrightarrow(t_2,x_2)\longrightarrow\cdots
```

Each arrow can replace many SSA reactions. The internal reactions have deliberately
not been reconstructed.
"""

# ╔═╡ 3e97f5ac-a058-463d-a335-c9ba763e92c4
md"""
## 1. Tian--Burrage toggle switch

The state is ``x=(U,V)``. The four reactions are production and degradation of
``U`` and ``V``, with mutual Hill repression.
"""

# ╔═╡ 2af97abf-0d72-42da-ad69-e9dca3a28b49
begin
    ...
    model = DiscreteStochasticSystem(reaction_network)
...
# ╔═╡ 1687850f-b63b-4bb2-9ff7-c1b9d696f7c7
md"""
## 2. ELSE mathematics

Let ``J`` be a small subnetwork containing the current state. Reactions leaving
``J`` are absorbed, giving the transient generator ``Q=A_{JJ}``.

Starting from probability vector ``p_0``,

```math
p_J(t)=e^{Qt}p_0.
```

The probability that the process is still inside ``J`` is

```math
S(t)=\mathbf 1^\top e^{Qt}p_0.
```

To sample the exit time, draw ``u\sim\mathrm{Uniform}(0,1)`` and solve

```math
S(\tau)=u.
```

At that sampled time, state ``j`` carries outward probability flux

```math
w_j(\tau)=\Delta_j\,[e^{Q\tau}p_0]_j,
```

where ``\Delta_j`` is the total reaction rate from state ``j`` to states outside
``J``. Sampling from these weights couples the exit location correctly to the
exit time.
"""

# ╔═╡ 36d38e88-cec2-4ab6-ae46-fb38bd8e3a4f
md"""
## 3. A small local subnetwork

For readability, use a Manhattan-radius neighborhood around the current state.
Increasing `radius` skips more SSA reactions per ELSE step, but makes each matrix
calculation larger.
"""

# ╔═╡ b76f037c-7145-416c-87b3-88e1c3d68314
function local_subnetwork(center, radius)
    space = StateSpace{CartesianIndex, Float64}()
    U₀, V₀ = center[1], center[2]

    for dU in -radius:radius
        for dV in -(radius - abs(dU)):(radius - abs(dU))
            U, V = U₀ + dU, V₀ + dV
            U >= 0 && V >= 0 && add_state!(space, CartesianIndex(U, V), 0.0)
        end
    end
...
# ╔═╡ 8ba8b094-84c8-42d3-a92b-74057aa91246
md"""
## 4. Two small sampling helpers
"""

# ╔═╡ 790de86c-8995-48cd-961e-94c76c71b15d
...
# ╔═╡ b8a24244-c9b3-412c-a9b2-833021b2aef0
function sample_exit_time(probability_at_time, mean_exit_time, rng)
    target_survival = rand(rng)

    # Find an interval containing the exit time.
    low = 0.0
    high = mean_exit_time
    while sum(probability_at_time(high)) > target_survival
        high *= 2
    end

    # Invert S(t) by bisection.
    for _ in 1:50
        middle = (low + high) / 2
        if sum(probability_at_time(middle)) > target_survival
            low = middle
        else
            high = middle
        end
...
# ╔═╡ 08309bb3-e9a7-4d43-8292-99433a567d50
md"""
## 5. One stochastic ELSE step

This function replaces the inner SSA reaction loop.

It returns:

- `waiting_time`: time until the local subnetwork is exited;
- `preexit`: state immediately before the exit reaction;
- `next_state`: state immediately after the exit reaction.
"""

# ╔═╡ f442b9b3-d2a5-4a60-9651-661444f123f2
function else_step(current_state, radius, model, rates, rng)
    # Build the killed generator for the local subnetwork.
    space = local_subnetwork(current_state, radius)
    Q_sparse, _, _, exit_rate =
        build_generator(space, model, rates, 0.0; absorbing=true)
    Q = Matrix(Q_sparse)

    p₀ = zeros(length(space))
    p₀[space.index[current_state]] = 1.0

    # Diagonalize once, then evaluate exp(Q*t)*p₀ cheaply during bisection.
    decomposition = eigen(Q)
    coefficients = decomposition.vectors \ p₀

    function probability_at_time(t)
        p = decomposition.vectors *
            (exp.(decomposition.values .* t) .* coefficients)
        max.(real.(p), 0.0)
    end

    # The fundamental matrix supplies a convenient initial time scale.
    occupation = -(Q \ p₀)
    mean_exit_time = sum(occupation)
    waiting_time =
        sample_exit_time(probability_at_time, mean_exit_time, rng)

    # Jointly sample the state from which the process exits.
    p_at_exit = probability_at_time(waiting_time)
    exit_weights = exit_rate .* p_at_exit
    preexit_index = sample_index(exit_weights, rng)
    preexit = space.states[preexit_index]

    # Select the actual reaction that leaves the local subnetwork.
    outside_states = CartesianIndex[]
    outside_rates = Float64[]

    for reaction in eachindex(model.stoichvecs)
        rate = Float64(model.propensities[reaction](preexit, rates, 0.0))
        destination = preexit + model.stoichvecs[reaction]

        if rate > 0 && !haskey(space.index, destination)
            push!(outside_states, destination)
            push!(outside_rates, rate)
        end
    end

    next_state = outside_states[sample_index(outside_rates, rng)]

    (
        waiting_time=waiting_time,
        preexit=preexit,
        next_state=next_state,
        subnetwork_size=length(space),
    )
end

# ╔═╡ 15384719-0785-45db-a6a5-a67dc25cb8d7
md"""
## 6. The ELSE sampling loop

Compare this with SSA:

```julia
while t < final_time
    # SSA would sample one holding time and one reaction here.
    step = else_step(...)
    t += step.waiting_time
...
Each loop iteration is one exact first exit from a local subnetwork.
"""

# ╔═╡ f207a92a-bff8-4a6b-99ea-cb60e9a40d9b
function else_trajectory(initial_state, final_time;
                         radius=3, seed=1, model=model, rates=rates)
    rng = MersenneTwister(seed)
    time = 0.0
    state = initial_state

    times = Float64[time]
    states = CartesianIndex[state]
    number_of_else_steps = 0

    while time < final_time
        step = else_step(state, radius, model, rates, rng)
        new_time = time + step.waiting_time
        new_time > final_time && break

        # Record both sides of the exit reaction.
        push!(times, new_time)
        ...
        time = new_time
        state = step.next_state
        number_of_else_steps += 1
    end

    (
        times=times,
        states=states,
        number_of_else_steps=number_of_else_steps,
    )
end

# ╔═╡ 039a7345-e544-4273-8a0a-8ae4d049c61b
md"""
## 7. Run several trajectories

These values are deliberately small enough to keep the notebook interactive.
Set `final_time = 30.0` for the time interval used in the article.
"""

# ╔═╡ 1b92b7fb-a217-440b-bb7f-e1e8dddf7f6c
begin
...
# ╔═╡ db8eb206-1486-41a3-b79a-a11d7770fd94
trajectories = [
    else_trajectory(
        initial_state,
        final_time;
...
# ╔═╡ 7f73074a-7c89-4edc-823f-b41518de1fc8
[
    (
        trajectory=i,
        ELSE_steps=trajectory.number_of_else_steps,
        plotted_points=length(trajectory.times),
    )
    for (i, trajectory) in enumerate(trajectories)
]

# ╔═╡ 1833ea65-20d5-4f4d-b109-803fab56ae35
md"""
## 8. Visualize the exit skeletons

Lines connect sampled ELSE exit points. They are not claims about the skipped
internal reaction-by-reaction path.
"""

# ╔═╡ d7fe79ab-8baa-477a-ab37-bb331f2243ab
let
    U_color = :steelblue
    V_color = :darkorange
    ...
    figure = plot(
        ...
        title="$(length(trajectories)) stochastic ELSE exit trajectories",
        size=(850, 500),
    )

    for (i, trajectory) in enumerate(trajectories)
        U_values = [state[1] for state in trajectory.states]
        V_values = [state[2] for state in trajectory.states]
...
# ╔═╡ 06500702-0478-40df-8b5f-59563096f118
let
    figure = plot(
        xlabel="U",
        ylabel="V",
		legend=false;
        title="ELSE trajectories in state space",
        size=(650, 500),
    )
...
# ╔═╡ 819a4627-2c96-4e2f-a6f5-256dad6fb0c1
md"""
## Interpretation

- The loop is stochastic and produces a different path for each seed.
- It samples the full first-exit-time law, rather than inserting the mean exit
  time.
- Exit location and exit time are sampled jointly.
- Increasing `radius` makes each ELSE step cover a larger collection of states.
- The result is an exact embedded trajectory at local exit times.
- If every individual reaction event is required, the skipped portion must be
  reconstructed, which gives up the main advantage over SSA.
"""
