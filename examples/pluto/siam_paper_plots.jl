...
# ╔═╡ 5b20c0fe-eef5-11ef-198b-f5452dcc0ced
using CairoMakie, JLD, LaTeXStrings, SparseArrays

# ╔═╡ 8414f824-5196-4510-b87f-8318fad40e62
lv_data_load = begin
	global lv_data = load("pluto_notebooks/lv_data.jld")
	global lv_ϵₜ  = lv_data["ϵₜ"]
	global lv_𝒮ₜ   = lv_data["size_𝒮ₜ"]
end

# ╔═╡ 5063aeb6-1812-4bf8-8837-23a6ec66db85
mm_data_load = begin
	global mv_data = load("pluto_notebooks/mm_data.jld")
	global mm_ϵₜ  = mv_data["ϵₜ"]
	global mm_𝒮ₜ   = mv_data["size_𝒮ₜ"]
end

# ╔═╡ 7bf7c0a2-36c2-4e71-a96e-aa6cd2ccd422
ts_data_load = begin
	global ts_data = load("pluto_notebooks/ts_data.jld")
	global ts_ϵₜ   = ts_data["ϵₜ"]
	global ts_𝒮ₜ   = ts_data["size_𝒮ₜ"]
end

# ╔═╡ c641bf0c-f0bd-47d5-998e-c1be1320489e
begin
	T = 0:0.005:30
	# Create figure with a 1x3 grid layout
	fig = Figure(size=(1500, 900))  # Wider figure to fit 3 plots
	
	hline_pts = [1e-5, 0.03,  1.0e-10]
	
	titles = ["Michaelis Menten", "Lotka Volterra", "Toggle Switch"]
	
	ylables = [L"Truncation Error ", "", ""]
	
	ylables1 = [L"Truncated Set Size ", "", ""]
	
	times = [0:0.1:200,0:0,0:0.5:50]
	
	# Function to create a single plot
	function create_plot(ax, T, m, hline_pts, title, label)
	    lines!(ax, m, color=:black)
	    hlines!(ax, [hline_pts], color=:black, linestyle=:dashdot, linewidth=4, 
	            label=L"$x^2$")
	    ax.ylabel = label
	    ax.title = title
	    ax.titlesize=30
	    ax.ylabelsize=30
	    #ax.limits = (nothing, (0.026, 0.0305))
	    axislegend(ax)
	end
	
	function create_plot2(ax, T, m, label)
	    lines!(ax, m, color=:black)
	    ax.xlabel = L"Time "
	    ax.ylabel = label
	    ax.ylabelsize=30
	...
	# Create three side-by-side axes
	for (i, m_) in enumerate([mm_𝒮ₜ[2:end], lv_𝒮ₜ, ts_𝒮ₜ])
	    ax = Axis(fig[2, i])  # Place in row 1, column i
	    create_plot2(ax, times[i], m_, ylables1[i])
	end
	
	# Save and display
	#save("lv_err_grid.pdf", fig)
	fig
end
...
manifest_format = "2.0"
project_hash = "efd9c4c11841fccb58a346e0edb2066b2ae4586f"
