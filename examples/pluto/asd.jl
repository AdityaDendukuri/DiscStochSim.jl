...
# ╔═╡ 2204ee4d-21d2-4287-a62e-541590b81832
begin
	# numerical libraries
...
# ╔═╡ b348b0bf-bdd3-440e-bbde-f7aebf991de6
rn = @reaction_network begin
    k₁, A --> B 
	k₂, B --> B + C
end

# ╔═╡ faa0000f-4fa0-467e-8572-f601bcad1157
model = DiscreteStochasticSystem(rn);

# ╔═╡ e4a272e5-50ee-4a87-825c-dcb7619aeef5
def_params = begin
	# reaction rates 
	rates = [1e-6, 1e-1]
end

# ╔═╡ 33687cb0-9d75-11f0-3c35-e1484961719a
"""
    reconstruct_MasterEquation_ordered(
        active_vec::Vector{E},     # <-- desired NEW order (from your expand!)
        S_old_vec::Vector{E},      # order that produced A_old
        A_old::SparseMatrixCSC{T,Int},
        model, rates, boundary_condition, t;
        recompute_retained::Bool = false,
        overlap_threshold::Real = 0.3
    ) -> (A, in_flow, out_flow)

Rebuilds the generator using `active_vec` as the NEW compact ordering.
Only retained↔retained entries are copied from A_old; everything touching NEW
states (new↔retained, retained↔new, new↔new) is assembled with O(n_new * deg).
Diagonal is recomputed as -sum(outgoing inside NEW set).
"""
function reconstruct_MasterEquation_ordered(
    active_vec::Vector{E},
    S_old_vec::Vector{E},
    A_old::SparseMatrixCSC{T,Int},
    model, rates, boundary_condition, t;
    ...
    # ---- maps for old/new compact indices
    new_index = Dict{E,Int}(s => i for (i,s) in enumerate(active_vec))
    old_index = Dict{E,Int}(s => i for (i,s) in enumerate(S_old_vec))

    S_new = Set(active_vec)
    S_old = Set(S_old_vec)

    retained = intersect(S_new, S_old)
    n_new    = length(active_vec)

    # If overlap too small, just rebuild fully (using your existing exact builder)
    if length(retained) < overlap_threshold * n_new
        # exact full build that respects order in `active_vec`
        # (this mirrors your MasterEquation but uses the provided order)
        # --- non-diagonal
        I = Int[]; J = Int[]; V = T[]
        state_id_map = new_index
        @inbounds for (i, xj) in enumerate(active_vec)   # destination row i
            # sources that lead to xj
            for (k, ν) in enumerate(model.stoichvecs)
                src = xj - ν
                if src ∈ S_new && boundary_condition(src)
                    j = state_id_map[src]
                    α = model.propensities[k](src, rates, t)
                    if i != j
        ...
        # --- diagonal
        diag = zeros(T, n_new)
        @inbounds for (j, xj) in enumerate(active_vec)   # column j: outflow from xj
            s = zero(T)
            for (k, ν) in enumerate(model.stoichvecs)
                tgt = xj + ν
                if tgt ∈ S_new && boundary_condition(tgt)
                    s += model.propensities[k](xj, rates, t)
                end
            end
            diag[j] = -s
        end
        A = sparse([I; collect(1:n_new)], [J; collect(1:n_new)], [V; diag], n_new, n_new)
        # in/out “flows”: split A into off-diagonal and diagonal parts
        in_flow  = A - spdiagm(0 => diag)
        out_flow = spdiagm(0 => diag)
        return A, in_flow, out_flow
    end

    # ---- fast path: copy retained↔retained from A_old, assemble anything with NEW
    rows_old = rowvals(A_old)
    vals_old = nonzeros(A_old)

    I = Int[]; J = Int[]; V = T[]
    colsum = zeros(T, n_new)  # to form diagonal as -sum(outgoing inside NEW)

    # helper: add off-diagonal (i←j) and accumulate for diag(j)
    @inline function add_edge!(i::Int, j::Int, v::T)
        if i != j && v != 0
    ...
    # 1) retained → retained : copy from old sparsity (off-diagonals only)
    @inbounds for s in retained
        j_old = old_index[s]
        j_new = new_index[s]
        for nz in nzrange(A_old, j_old)
            i_old = rows_old[nz]
            if i_old == j_old; continue; end  # skip old diagonal
            dst = S_old_vec[i_old]
            if dst ∈ S_new
                i_new = new_index[dst]
                add_edge!(i_new, j_new, vals_old[nz])
            end
        end
    end

    # 2) retained → new : assemble from model
    @inbounds for s in retained
        j_new = new_index[s]
        # outgoing from s to targets that are NEW this step
        for (k, ν) in enumerate(model.stoichvecs)
            tgt = s + ν
            if tgt ∈ S_new && !(tgt ∈ retained) && boundary_condition(tgt)
                i_new = new_index[tgt]
                α = model.propensities[k](s, rates, t)
                add_edge!(i_new, j_new, α)
            end
        end
    end

    # 3) new → retained : assemble from model (sources that are NEW)
    @inbounds for s in S_new
        if s ∈ retained; continue; end
        j_new = new_index[s]
        for (k, ν) in enumerate(model.stoichvecs)
            src = s - ν
            if src ∈ S_new && (src ∈ retained) && boundary_condition(src)
                i_new = new_index[src]  # i_new is "from src" (column = src)
                α = model.propensities[k](src, rates, t)
                # edge is (s ← src) meaning (row=new_index[s], col=new_index[src])
                add_edge!(new_index[s], i_new, α)
            end
        end
    end

    # 4) new → new : assemble from model
    @inbounds for s in S_new
        if s ∈ retained; continue; end
        j_new = new_index[s]
        for (k, ν) in enumerate(model.stoichvecs)
    ...
    # 5) diagonal = - column sum of off-diagonals (probability conserving)
    diag = -colsum
    A = sparse([I; collect(1:n_new)], [J; collect(1:n_new)], [V; diag], n_new, n_new)
...
# ╔═╡ c464109e-7d76-4c12-b4a8-52a33d4ab773
size_St

# ╔═╡ 9c211848-c9bd-496c-9d11-b40d4efeaf97
function mean_sol(sol)
    sol_mean = map(sol) do (states, probs)
        sum(collect(Tuple(s)) .* p for (s, p) in zip(states, probs))
    end
    hcat(sol_mean...)'
end

# ╔═╡ 274ce29b-6f85-4de5-abb2-fdd23bca76f0
mean_sol(sol)

# ╔═╡ 57aa3ac8-c9b5-4864-a9f4-4399111ca643
begin
	# Check final state
	states, probs = sol[end]
	println("\nFinal iteration (10001):")
	println("States with B >= 1:")
	for (s, p) in zip(states, probs)
	    if s[2] >= 1 && p > 0.001
	        println("  State $s: prob = $p, B = $(s[2])")
	    end
	end
	
	# Check if there's a state with B=2
	has_B2 = any(s[2] == 2 for s in states)
	println("Has state with B=2? $has_B2")
end

# ╔═╡ e0f0a1c9-a24a-4a2c-8447-3073690d4fb8
begin
	function analyze_marginals(sol, time_indices=[1, 100, 1000, 10001])
	    for idx in time_indices
	        states, probs = sol[idx]
	        
	        # Extract marginals for each species
	        marginal_B = Dict{Int, Float64}()
	        
	        for (state, prob) in zip(states, probs)
	            B = state[2]
	            marginal_B[B] = get(marginal_B, B, 0.0) + prob
	        end
	        ...
	        for (b_val, prob) in sort(collect(marginal_B))
	            println("  B=$b_val: probability = $prob")
	        end
	        
	        mean_B = sum(k*v for (k,v) in marginal_B)
	        println("\nMean B (from marginal): $mean_B")
	        println("Mean B (direct calc): ", sum(s[2]*p for (s,p) in zip(states, probs)))
	    end
	end
	
	analyze_marginals(sol)
end

# ╔═╡ 30a9d947-4d76-47ae-8e5c-deb3c6a35492
plot_marginals(sol)

# ╔═╡ f8c9ae8b-bfce-409c-b4da-f1370ef842a3
"""
Main simulation using HybridStateSpace with matrix reconstruction
"""
fsp_sim_hybrid = begin
    # Initialize
    ...
    @progress for (iter, t) in enumerate(T)
        # 1. Expand state space (fast Set-based)
        expand!(space, model, 1)
        size_St[iter] = length(space.state_vec)
        
        # 2. Build or reconstruct matrix
        builder = HybridMasterEquationBuilder(space, model, rates)
        
        #if iter == 1 || isempty(global_old)
            A = build_sparse_matrix(builder, t)
        #else
        #    A = reconstruct_sparse_matrix(builder, A_old, global_old, t)
        #end
        
        # 3. Evolve probability (A is already compact size)
        p_new = expv(δt, A, space.probabilities)
        ...
        # 6. Compress with flux protection
        stats = compress!(space, model, rates, t, 0.99; flux_tolerance=1e-2)
        
        # 7. Renormalize
        total = renormalize!(space)
		if iter % 100 == 0
    println("Iter $iter: total before norm = $total, after norm = $(sum(space.probabilities))")
end
        
        # 8. Store diagnostics
        ϵₜ[iter] = 1.0 - total
        
        # 9. Save for next reconstruction (global indices)
        global global_old = get_global_indices(space)
        global A_old = A
    end
    
    (sizes=size_St, errors=ϵₜ, solution=sol)
end

# ╔═╡ 1bf961c0-a71c-434a-872c-2916d24af393
# ╠═╡ disabled = true
#=╠═╡
"""
Main simulation using StateSpace with matrix reconstruction (flux-aware FSP)
"""
fsp_sim = begin
    # ---------------- Init ----------------
    U₀ = CartesianIndex(1, 0, 0)

    # renamed: HybridStateSpace -> StateSpace
    space = StateSpace{CartesianIndex{3}, Float64}()

    # Set initial state
    DiscStochSim.add_state!(space, U₀, 1.0)
    ...
    size_St = zeros(Int, length(T))
    ϵₜ     = zeros(length(T))
    sol    = Vector{Tuple{Vector{CartesianIndex{3}}, Vector{Float64}}}()

    # For reconstruction carry-over
    global_old = Int[]
    A_old = spzeros(Float64, 0, 0)
    ...
    for (iter, t) in enumerate(T)
        # 1) Expand state space (fast Set-based)
        # old call was expand!(space, model, 1); new signature uses keyword
        DiscStochSim.expand!(space, model; depth=1)
        size_St[iter] = length(space.state_vec)

        # 2) Build or reconstruct matrix
        # renamed: HybridMasterEquationBuilder -> MasterEquationBuilder
        builder = MasterEquationBuilder(space, model, rates)

        A =
            if iter == 1 || isempty(global_old)
                DiscStochSim.build_sparse_matrix(builder, t)
            else
                # column-wise, diagonal from kept off-diagonals
                DiscStochSim.reconstruct_sparse_matrix(builder, A_old, global_old, t; recompute_retained=false)
            end
        ...
        # 5) Store solution snapshot
        push!(sol, (copy(space.state_vec), copy(space.probabilities)))

        # 6) Compress with flux protection
        DiscStochSim.compress!(space, model, rates, t, 0.99; flux_tolerance=1e-4)

        # 7) Renormalize
        total = DiscStochSim.renormalize!(space)
        if iter % 100 == 0
            println("Iter $iter: total before norm = $total, after norm = $(sum(space.probabilities))")
        end
        ...
        # 9) Save for next reconstruction (global indices)
        global_old = DiscStochSim.get_global_indices(space)
        A_old = A
    end

    (sizes=size_St, errors=ϵₜ, solution=sol, times=collect(T))
end
