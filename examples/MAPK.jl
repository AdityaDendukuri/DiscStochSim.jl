...
rn_mapk = @reaction_network begin
        # First phosphorylation: S + K ⇌ S·K → Sp + K
        k1f, S + K --> SK
        k1r, SK --> S + K
        k1cat, SK --> Sp + K
        
        # Second phosphorylation: Sp + K ⇌ Sp·K → Spp + K
        k2f, Sp + K --> SpK
        k2r, SpK --> Sp + K
        k2cat, SpK --> Spp + K
        
        # First dephosphorylation: Spp + P ⇌ Spp·P → Sp + P
        k3f, Spp + P --> SppP
        k3r, SppP --> Spp + P
        k3cat, SppP --> Sp + P
        
        # Second dephosphorylation: Sp + P ⇌ Sp·P → S + P
        k4f, Sp + P --> SpP
        k4r, SpP --> Sp + P
        k4cat, SpP --> S + P
    end
...
begin
    ...
    U₀ = CartesianIndex(S_total, K_total, 0, 0, 0, 0, P_total, 0, 0)
    
    # Rate constants: [k_bind, k_unbind, k_cat] for each of the 4 cycles
    rates = [
        0.001, 0.001, 0.1,  # S + K ⇌ S·K → Sp + K
        0.001, 0.001, 0.1,  # Second phosphorylation (Sp -> Spp)
        0.001, 0.001, 0.1,  # First dephosphorylation (Spp -> Sp)
        0.001, 0.001, 0.1   # Second dephosphorylation (Sp -> S)
    ]
    
    bounds = (0, S_total + K_total + P_total + 5) 
...
begin
	...
	final_states = collect(sol[end][1])
	final_probs = sol[end][2]
	...
	n_species = 9
	species_names = ["S", "K", "SK", "Sp", "SpK", "Spp", "P", "SppP", "SpP"]
	...
	lines!(ax_size, sol_t, sol_S_size, color=:blue, linewidth=2)
	scatter!(ax_size, sol_t[1:5:end], sol_S_size[1:5:end], color=:blue, markersize=4)
	...
	lines!(ax_dt, time_for_dt, dt_values, color=:red, linewidth=2)
	scatter!(ax_dt, time_for_dt[1:5:end], dt_values[1:5:end], color=:red, markersize=4)
	...
	for (i, species_idx) in enumerate(key_species_indices)
		lines!(ax_mean, sol_t, mean_trajectories[:, species_idx], 
