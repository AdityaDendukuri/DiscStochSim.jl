...
# ╔═╡ c3000001-3000-4000-8000-000000000001
begin
...
# ╔═╡ c3000002-3000-4000-8000-000000000002
md"""
# Population ELSE for the Tian--Burrage toggle

This notebook never samples hundreds or millions of ELSE trajectories
independently. It carries one vector of trajectory counts and constructs one
sampled population-transfer matrix per switching leg.

We use the FSP column convention

\$\$
\frac{d\boldsymbol p}{dt}=A_{JJ}\boldsymbol p.
\$\$

Mark's ELSE rate matrix is
...
In FSP orientation, the corresponding fundamental matrix is

\$\$
Z_A=-A_{JJ}^{-1}=Z^{\mathsf T}.
\$\$

Let \$E\$ inject coarse entry states into the basin and let \$B\$ contain rates
from internal states to entries in the opposite basin. The coarse transition
matrix is
...
# ╔═╡ c3000003-3000-4000-8000-000000000003
begin
    burrage_reactions = @reaction_network begin
        @species U(t) V(t)
    ...
    burrage_model = DiscreteStochasticSystem(burrage_reactions)
    ...
    burrage_rates = collect(values(burrage_parameters))
end

# ╔═╡ c3000004-3000-4000-8000-000000000004
begin
    u_birth(v, p) =
        p.η * (p.α1 + p.β1 * p.K1^3 / (p.K1^3 + v^3))

    u_death(u, p) = (p.d1 + p.dU) * u

    v_birth(u, p) =
        p.η * (p.α2 + p.β2 * p.K2^3 / (p.K2^3 + u^3))

    v_death(v, p) = p.d2 * v
end

# ╔═╡ c3000005-3000-4000-8000-000000000005
md"""
## Two physical basins
...
Both reactions that cross the diagonal are retained. From \$J_U\$ these are a
\$V\$ birth and a \$U\$ death. From \$J_V\$ they are a \$U\$ birth and a \$V\$
death.

The entry coordinate \$x\$ represents
...
# ╔═╡ c3000006-3000-4000-8000-000000000006
function burrage_basin_space(u_high, grid_limit)
...
# ╔═╡ c3000007-3000-4000-8000-000000000007
function build_population_basin(
    u_high,
    grid_limit,
    model,
    rates,
    ...
    states = burrage_basin_space(u_high, grid_limit)

    A, _, _, _ = build_generator(
        states,
        model,
        rates,
        0.0;
    ...
    entry_coordinates = collect(0:(grid_limit - 1))
    number_of_entries = length(entry_coordinates)
    number_of_states = length(states)

    E = spzeros(Float64, number_of_states, number_of_entries)
    B = spzeros(Float64, number_of_entries, number_of_states)

    for (column, x) in enumerate(entry_coordinates)
        entry = u_high ?
            CartesianIndex(x + 1, x) :
            CartesianIndex(x, x + 1)

        E[states.index[entry], column] = 1.0
    end

    for x in 0:grid_limit
        diagonal = states.index[CartesianIndex(x, x)]

        if u_high
            # V birth: (x,x) → (x,x+1)
            x < grid_limit &&
                (B[x + 1, diagonal] += v_birth(x, parameters))

            # U death: (x,x) → (x-1,x)
            x > 0 &&
                (B[x, diagonal] += u_death(x, parameters))
        else
            # U birth: (x,x) → (x+1,x)
            x < grid_limit &&
                (B[x + 1, diagonal] += u_birth(x, parameters))

            # V death: (x,x) → (x,x-1)
            x > 0 &&
                (B[x, diagonal] += v_death(x, parameters))
        end
    end
...
# ╔═╡ c3000008-3000-4000-8000-000000000008
md"""
## Constructing \$M\$ by columns

For entry column \$a\$, only one solve is required:
...
The exit-probability column is
...
Columns are computed only when their entry states contain trajectories, then
cached and reused.
"""

# ╔═╡ c3000009-3000-4000-8000-000000000009
function factor_population_basin(basin)
    (
        basin...,
        Z_solve=lu(-basin.A),
        M_columns=Dict{Int, Vector{Float64}}(),
    )
end

# ╔═╡ c300000a-3000-4000-8000-00000000000a
function M_column!(basin, source_column)
    get!(basin.M_columns, source_column) do
        occupation =
            basin.Z_solve \ Vector(basin.E[:, source_column])

        probability = max.(basin.B * occupation, 0.0)
        probability
    end
end

# ╔═╡ c300000b-3000-4000-8000-00000000000b
function initial_M_column(basin, initial_state)
    p0 = zeros(length(basin.states))
    p0[basin.states.index[initial_state]] = 1.0

    occupation = basin.Z_solve \ p0
    max.(basin.B * occupation, 0.0)
end

# ╔═╡ c300000c-3000-4000-8000-00000000000c
md"""
## One multinomial draw per occupied column

If \$n_a\$ trajectories enter through column \$a\$, construct its sampled
population column as

\$\$
M^{(N)}_{\cdot a}
\sim
\operatorname{Multinomial}
\left(n_a,M_{\cdot a}\right).
\$\$

Column sums equal the incoming population. Row sums form the next population.
No individual trajectory loop is used.
"""

# ╔═╡ c300000d-3000-4000-8000-00000000000d
function sample_population_column(probability, population, rng)
    physical_probability = max.(probability, 0.0)
    missing_probability =
        max(1.0 - sum(physical_probability), 0.0)

    probabilities =
        [physical_probability; missing_probability]
    probabilities ./= sum(probabilities)

    sampled = rand(
        rng,
        Multinomial(population, probabilities),
    )

    (
        counts=sampled[1:end-1],
        outside_grid=sampled[end],
    )
end

# ╔═╡ c300000e-3000-4000-8000-00000000000e
function construct_population_M(basin, population, rng)
    number_of_entries = length(population)
    M_counts = zeros(Int, number_of_entries, number_of_entries)
    outside_grid = 0

    for source in findall(>(0), population)
        probability = M_column!(basin, source)
        sampled = sample_population_column(
            probability,
            population[source],
            rng,
        )

        M_counts[:, source] .= sampled.counts
        outside_grid += sampled.outside_grid
    end

    next_population = vec(sum(M_counts; dims=2))
...
# ╔═╡ c300000f-3000-4000-8000-00000000000f
function burrage_population_experiment(
    ensemble_size,
    number_of_switches,
    grid_limit;
    initial_state=CartesianIndex(85, 5),
    seed=2026,
)
    rng = MersenneTwister(seed)

    setup_seconds = @elapsed begin
        U_basin = factor_population_basin(
            build_population_basin(
                true,
                grid_limit,
                burrage_model,
                burrage_rates,
        ...
        V_basin = factor_population_basin(
            build_population_basin(
                false,
                grid_limit,
                burrage_model,
    ...
    history = NamedTuple[]
    count_matrices = Matrix{Int}[]

    population_seconds = @elapsed begin
        first_probability =
            initial_M_column(U_basin, initial_state)

        first_sample = sample_population_column(
            first_probability,
            ensemble_size,
            rng,
        )

        population = first_sample.counts

        push!(
            history,
            (
                switch=1,
                entered_basin=:V_high,
                population=copy(population),
                outside_grid=first_sample.outside_grid,
            ),
        )

        for switch in 2:number_of_switches
            source_basin = iseven(switch) ? V_basin : U_basin
            entered_basin = iseven(switch) ? :U_high : :V_high

            sampled_M = construct_population_M(
                source_basin,
                population,
                rng,
            )

            push!(count_matrices, sampled_M.M_counts)
            population = sampled_M.next_population
            ...
            push!(
                ...
                (
                    ...
                    population=copy(population),
                    outside_grid=sampled_M.outside_grid,
                ),
            )
        end
    end
    ...
    (
        ...
        first_counts=history[1].population,
        history=history,
...
# ╔═╡ c3000010-3000-4000-8000-000000000010
md"""
## Controlled full-basin run

The physical Burrage basin with `grid_limit = 680` contains about 232,000 states
per basin. It is disabled initially. Setting `run_population = true` builds two
sparse factorizations and then constructs only the occupied columns of \$M\$.

Changing `ensemble_size` from \$10^3\$ to \$10^6\$ changes the multinomial counts,
not the number of linear solves.
"""

# ╔═╡ c3000011-3000-4000-8000-000000000011
begin
    run_population = true
...
# ╔═╡ c3000012-3000-4000-8000-000000000012
population_result = if run_population
    burrage_population_experiment(
        ensemble_size,
        number_of_switches,
        grid_limit,
    )
...
# ╔═╡ c3000013-3000-4000-8000-000000000013
if population_result === nothing
    md"""
    Population calculation is paused. Set `run_population = true` in the cell
    above when ready.
    """
else
    [
        (
            switch=leg.switch,
            entered_basin=leg.entered_basin,
            trajectories=sum(leg.population),
            occupied_entry_states=count(>(0), leg.population),
            outside_grid=leg.outside_grid,
        )
        for leg in population_result.history
    ]
end

# ╔═╡ c3000014-3000-4000-8000-000000000014
if population_result === nothing
    nothing
else
    let
        figure = plot(
            xlabel="diagonal entry coordinate",
            ylabel="trajectory count",
            title="Burrage population propagated through sampled M",
            legend=:topright,
            size=(820, 480),
        )
        ...
        for leg in population_result.history
            plot!(
                figure,
                0:(length(leg.population) - 1),
                leg.population;
                lw=2,
                label="switch $(leg.switch): $(leg.entered_basin)",
            )
        end
...
# ╔═╡ c3000015-3000-4000-8000-000000000015
if population_result === nothing
    nothing
else
    (
        ensemble_size=ensemble_size,
        basin_states=length(population_result.U_basin.states),
        setup_seconds=population_result.setup_seconds,
        population_seconds=population_result.population_seconds,
        U_columns_of_M=length(population_result.U_basin.M_columns),
        V_columns_of_M=length(population_result.V_basin.M_columns),
    )
end

# ╔═╡ c3000016-3000-4000-8000-000000000016
md"""
## Fourier stage

Once the sampled matrices have been constructed, temporal information can be
added without recovering particle labels. The transformed population matrix is
...
For a sampled count \$M^{(N)}_{ba}\$, its conditional time transform is obtained
from

\$\$
\widehat f_{ba}(s)
=
\frac{\widehat M_{ba}(s)}{M_{ba}}.
\$\$

Therefore a count-weighted transformed column is
...
These columns can be convolved by matrix multiplication and inverted once for
the ensemble. This stage is deliberately separate from construction of the
discrete population matrices.
"""

# ╔═╡ c3000017-3000-4000-8000-000000000017
function conditional_transform(transformed_column, M_column)
    result = zeros(ComplexF64, length(M_column))

    for destination in eachindex(M_column)
        if M_column[destination] > eps(Float64)
            result[destination] =
                transformed_column[destination] /
                M_column[destination]
        end
    end
...
# ╔═╡ c3000018-3000-4000-8000-000000000018
function sampled_population_transform(
    result,
    s,
    number_of_switches,
)
    U_factor = lu(s * I - result.U_basin.A)
    V_factor = lu(s * I - result.V_basin.A)

    p0 = zeros(ComplexF64, length(result.U_basin.states))
    p0[result.U_basin.states.index[result.initial_state]] = 1.0

    first_transformed =
        result.U_basin.B * (U_factor \ p0)

    first_conditional = conditional_transform(
        first_transformed,
        result.first_probability,
    )

    ensemble_size =
        sum(result.first_counts) +
        result.history[1].outside_grid

    arrival =
        (result.first_counts ./ ensemble_size) .*
        first_conditional

    switch_transforms =
        Vector{ComplexF64}(undef, number_of_switches)
    switch_transforms[1] = sum(arrival)
    ...
    for switch in 2:number_of_switches
        ...
        factorization = iseven(switch) ?
            V_factor :
            U_factor

        source_population =
            result.history[switch - 1].population
        M_counts =
            result.count_matrices[switch - 1]

        next_arrival =
            zeros(ComplexF64, length(source_population))

        for source in findall(>(0), source_population)
            entry = zeros(
                ComplexF64,
                length(basin.states),
            )
            entry_index =
                findnz(basin.E[:, source])[1][1]
            entry[entry_index] = 1.0

            transformed_column =
                basin.B * (factorization \ entry)

            probability_column =
                M_column!(basin, source)

            time_transform = conditional_transform(
                transformed_column,
                probability_column,
            )
        ...
        switch_transforms[switch] = sum(arrival)
    end

    switch_transforms
end

# ╔═╡ c3000019-3000-4000-8000-000000000019
function population_transform_grid(
    result,
    nodes,
    number_of_switches,
)
    transformed = Matrix{ComplexF64}(
        undef,
        length(nodes),
        number_of_switches,
    )
...
# ╔═╡ c300001a-3000-4000-8000-00000000001a
function inverse_fourier_cdf(
    transform_values,
    nodes,
    time;
    period,
    ...
    cdf_transform = transform_values ./ nodes
    fourier_sum = 0.5 * real(cdf_transform[1])
    ...
    value = exp(shift * time) * fourier_sum / period
    clamp(value, 0.0, total_mass)
end

# ╔═╡ c300001b-3000-4000-8000-00000000001b
begin
    run_fourier = false

    number_of_fourier_nodes = 8
    time_horizon = 1.0e15
    fourier_period = 2 * time_horizon
    fourier_shift = 6 / fourier_period
end

# ╔═╡ c300001c-3000-4000-8000-00000000001c
fourier_result = if run_fourier && population_result !== nothing
    elapsed_seconds = @elapsed begin
        fourier_nodes = [
            fourier_shift + im * k * π / fourier_period
            for k in 0:number_of_fourier_nodes
        ]

        transformed_population =
            population_transform_grid(
                population_result,
                fourier_nodes,
                number_of_switches,
            )
        ...
        time_grid =
            range(0.0, time_horizon; length=300)

        total_mass = [
            sum(leg.population) / ensemble_size
            for leg in population_result.history
        ]
        ...
        switch_cdf = [
            [
                inverse_fourier_cdf(
                    transformed_population[:, switch],
                    fourier_nodes,
                    time;
                    period=fourier_period,
                    shift=fourier_shift,
                    total_mass=total_mass[switch],
                )
    ...
    (
        nodes=fourier_nodes,
        transformed=transformed_population,
        time_grid=time_grid,
        total_mass=total_mass,
        switch_cdf=switch_cdf,
...
# ╔═╡ c300001d-3000-4000-8000-00000000001d
if population_result === nothing
    md"""
    Construct the sampled population matrices first by setting
    `run_population = true`.
    """
elseif !run_fourier
    md"""
    The sampled population matrices are ready. Set `run_fourier = true` to add
    conditional timing and perform the ensemble Fourier inversion.
    """
else
    let
        figure = plot(
            xlabel="time",
            ylabel="expected trajectory count",
            title="Burrage population ELSE with Fourier timing",
            legend=:bottomright,
            size=(820, 480),
        )
        ...
        for switch in 1:number_of_switches
            plot!(
                figure,
                fourier_result.time_grid,
                ensemble_size .* fourier_result.switch_cdf[switch];
                lw=2.5,
                label="switch $switch",
            )
        end
...
# ╔═╡ c300001e-3000-4000-8000-00000000001e
if fourier_result === nothing
    nothing
else
    (
        ensemble_size=ensemble_size,
        fourier_nodes=length(fourier_result.nodes),
        switches=number_of_switches,
        fourier_seconds=fourier_result.elapsed_seconds,
        eventual_switch_mass=fourier_result.total_mass,
    )
end
