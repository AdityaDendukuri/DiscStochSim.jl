...
# ╔═╡ e89d8f53-c9d9-4c66-9126-95812f9a16f8
"""
    expand1_adaptive!(X, p, model, rates, t, boundary_condition; flux_tolerance=1e-9)
...
function expand1_adaptive!(
    ...
    # 1. First, calculate the total flux of the entire system for reference.
    #    This is the sum of all fluxes from all states.
    total_flux = zero(T)
    for (i, state) in enumerate(X_vec)
        # The weight w(x) is the sum of all outgoing propensities from this state
        weight = sum(prop(state, rates, t) for prop in model.propensities)
        total_flux += p[i] * weight
    end
    
    # 2. Define the adaptive threshold based on the total system activity.
    flux_threshold = total_flux * flux_tolerance

    # 3. Perform the adaptive expansion.
    #    Iterate over a copy so we can safely modify X.
    for (i, x) in enumerate(X_vec)
        # Check each possible reaction leaving state x
        for (j, prop) in enumerate(model.propensities)
            # Calculate the flux of this specific reaction
            reaction_propensity = prop(x, rates, t)
            reaction_flux = p[i] * reaction_propensity
            
            # --- The Adaptive Filter ---
            # Only expand if this reaction's flux is above the adaptive threshold.
            if reaction_flux > flux_threshold
                new_x = x + model.stoichvecs[j]
                
                # Add the new state if it's within bounds
...
fsp_sim_rober = begin
    ...
    # Initial small step/ evolution
    local δt = 1e-6
	#global 𝒮ₜ, pₜ = expand!(𝒮ₜ, pₜ, model, boundary_condition, 5)	
	#A = MasterEquation(𝒮ₜ, model, rates, boundary_condition, t)
	#pₜ = expv(δt, A, pₜ)
    # --- Main Adaptive Loop ---
    while t < tf
        # 1. Expand state space (before A to keep indexing valid)
        ...
        # 6. Prune low-mass / low-flux states
