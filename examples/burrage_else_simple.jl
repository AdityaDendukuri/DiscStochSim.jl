...
md"""
**ELSE** restricts the master-equation generator to one basin and, in a *single
linear solve*, returns the mean flip time and how the trajectory fills that basin.
Solve each basin and you see both — the bistability.
"""
...
Tian–Burrage toggle: `U` and `V` mutual Hill repression plus degradation. We start
in the U-high state at `(85, 5)`.
"""
...
Restrict the generator to a basin `J` (every reaction that leaves `J` is absorbed)
→ the matrix `A_JJ`. Its inverse is the **fundamental matrix** `Z = -inv(A_JJ)`.
One solve gives the **occupation**

```math
\mathrm{occ} = Z\,\mathbf{p}_0 = -\mathbf{A}_{JJ}^{-1}\mathbf{p}_0
```

— the expected time spent in each state before the flip. The **mean flip time** is
`sum(occ)`.
"""

# ╔═╡ b1000009-0000-4000-8000-000000000009
function else_basin(u_high; G=680)
    sp = StateSpace{CartesianIndex, Float64}()
    for a in 0:G, b in 0:G
        (u_high ? a >= b : b >= a) && add_state!(sp, CartesianIndex(a, b), 0.0)
    end
    A, _, _, Δ = build_generator(sp, model, rates, 0.0; absorbing=true)   # A_JJ (sparse)
    start = u_high ? CartesianIndex(85, 5) : CartesianIndex(5, 85)
    p0 = zeros(length(sp)); p0[sp.index[start]] = 1.0
    occ = -(A \ p0)                                                       # Z p0 — one solve
    (sp=sp, occ=occ, mean_flip=sum(occ))
end

# ╔═╡ b100000a-0000-4000-8000-00000000000a
md"## Mean flip time (one solve per basin)"

# ╔═╡ b100000b-0000-4000-8000-00000000000b
Uwell = else_basin(true)

# ╔═╡ b100000c-0000-4000-8000-00000000000c
Vwell = else_basin(false)

# ╔═╡ b100000d-0000-4000-8000-00000000000d
(U_high_flip_time = Uwell.mean_flip, V_high_flip_time = Vwell.mean_flip)

# ╔═╡ b100000e-0000-4000-8000-00000000000e
md"""
## Bistability — ELSE finds both basins

The occupation is where the trajectory spends its time before flipping — the shape
of each basin. Overlaying both ELSE solves (log scale) shows the two wells: one
solve found U-high, the other found V-high.
"""

# ╔═╡ b100000f-0000-4000-8000-00000000000f
let
	G = 680
	field = fill(NaN, G+1, G+1)  # field[U+1, V+1] = log10 occupation
	for well in (Uwell, Vwell)
		m = maximum(well.occ)
		for (s, o) in zip(well.sp.states, well.occ)
			o > 0 && (field[s[1]+1, s[2]+1] = log10(o/m))
		end
	end
	Plots.heatmap(0:G, 0:G, permutedims(field); clims=(-6, 0), c=:viridis,
	        xlabel="U", ylabel="V", title="ELSE occupation of both basins (bistability)",
	        size=(560, 520), colorbar_title="log₁₀ occupation")
end
