...
# ╔═╡ b2000001-2000-4000-8000-000000000001
begin
...
# ╔═╡ b2000002-2000-4000-8000-000000000002
md"""
# Batched ELSE for the toggle switch

This notebook computes ensemble switching statistics without constructing
individual trajectories.

For one basin with killed generator \$Q\$, let \$E\$ inject the possible entry
states and let \$R\$ map internal escape states to entries in the opposite basin.
After swapping the species labels, both toggle basins use the same canonical
subnetwork.

The transformed coarse transition kernel is
...
which is the discrete ELSE exit kernel. At nonzero \$s\$, \$K(s)\$ contains both
the exit probabilities and the holding-time information.
"""

# ╔═╡ b2000003-2000-4000-8000-000000000003
hill_rate(z, production, threshold, exponent) =
    production * threshold^exponent /
    (threshold^exponent + z^exponent)

# ╔═╡ b2000004-2000-4000-8000-000000000004
toggle_reactions = @reaction_network begin
    hillr(Y, production, threshold, exponent), 0 --> X
    hillr(X, production, threshold, exponent), 0 --> Y
    degradation, X --> 0
    degradation, Y --> 0
end

# ╔═╡ b2000005-2000-4000-8000-000000000005
toggle_model = DiscreteStochasticSystem(toggle_reactions)

# ╔═╡ b2000006-2000-4000-8000-000000000006
function x_high_mode(production, threshold, exponent, degradation)
    x, y = float(production), 0.0

    for _ in 1:2000
        x = hill_rate(y, production, threshold, exponent) / degradation
        y = hill_rate(x, production, threshold, exponent) / degradation
    end

    CartesianIndex(round(Int, x), round(Int, y))
end

# ╔═╡ b2000007-2000-4000-8000-000000000007
function toggle_basin(grid_limit)
    states = StateSpace{CartesianIndex, Float64}()

    for x in 0:grid_limit
        for y in 0:x
            add_state!(states, CartesianIndex(x, y), 0.0)
        end
    end
...
# ╔═╡ b2000008-2000-4000-8000-000000000008
md"""
## Canonical boundary states

An escape from \$(x,x)\$ enters the opposite basin at \$(x,x+1)\$. Swapping
\$X\$ and \$Y\$ maps this to the canonical entry state \$(x+1,x)\$.

Therefore the two-dimensional basin is replaced by a one-dimensional collection
of boundary entry coordinates.
"""

# ╔═╡ b2000009-2000-4000-8000-000000000009
function fourier_subnetwork(
    grid_limit,
    production,
    threshold,
    exponent,
    ...
    states = toggle_basin(grid_limit)
    parameters = [production, threshold, exponent, degradation]

    Q, _, _, _ = build_generator(
        states,
        toggle_model,
        parameters,
        0.0;
    ...
    R = spzeros(Float64, number_of_entries, number_of_states)

    for (column, x) in enumerate(entry_coordinates)
        entry_state = states.index[CartesianIndex(x + 1, x)]
        E[entry_state, column] = 1.0
    end

    for x in 0:grid_limit
        exit_state = states.index[CartesianIndex(x, x)]

        if x < grid_limit
            # Y birth: (x,x) → (x,x+1), then swap → (x+1,x)
            R[x + 1, exit_state] +=
                hill_rate(x, production, threshold, exponent)
        end

        if x > 0
            # X death: (x,x) → (x-1,x), then swap → (x,x-1)
            R[x, exit_state] += degradation * x
        end
    end

    initial_state = x_high_mode(
        production,
        threshold,
        exponent,
        degradation,
...
# ╔═╡ b200000a-2000-4000-8000-00000000000a
begin
    production = 20.0
    threshold = 10.0
    exponent = 3.0
    ...
    fourier_model = fourier_subnetwork(
        grid_limit,
        production,
        threshold,
        exponent,
    ...
    (
        basin_states=length(fourier_model.states),
        boundary_entries=length(fourier_model.entry_coordinates),
        initial_state=fourier_model.initial_state,
    )
end

# ╔═╡ b200000b-2000-4000-8000-00000000000b
md"""
## Transforming several trajectory legs
...
Successive switches are obtained without listing individual paths:

\$\$
k_\ell(s)=K(s)^{\ell-1}k_1(s).
\$\$

The scalar transform of the time of the \$\ell\$-th switch is

\$\$
\widehat f_\ell(s)=\mathbf 1^\mathsf{T}k_\ell(s).
\$\$

Matrix multiplication sums over all intermediate boundary states automatically.
"""

# ╔═╡ b200000c-2000-4000-8000-00000000000c
function transformed_switches(model, s, number_of_switches)
    operator = s * I - model.Q

    # One factorization at this transform point, with all right-hand sides.
    right_hand_sides =
        Matrix{ComplexF64}(hcat(model.initial, model.E))
    solutions = operator \ right_hand_sides

    first_switch = model.R * solutions[:, 1]
    kernel = model.R * solutions[:, 2:end]

    result = Vector{ComplexF64}(undef, number_of_switches)
    arrival = first_switch

    for switch in 1:number_of_switches
        result[switch] = sum(arrival)
        arrival = kernel * arrival
    end

    result
end

# ╔═╡ b200000d-2000-4000-8000-00000000000d
function transform_grid(model, nodes, number_of_switches)
    ...
    for (row, s) in enumerate(nodes)
        values[row, :] .= transformed_switches(
            model,
            s,
            number_of_switches,
        )
...
# ╔═╡ b200000e-2000-4000-8000-00000000000e
md"""
## Fourier inversion

The Laplace transform of a CDF is \$\widehat f(s)/s\$. Using nodes
...
the Fourier-series inversion is

\$\$
F(t)\approx
\frac{e^{at}}{T}
\left[
\frac{1}{2}\operatorname{Re}\widehat F(a)
+
\sum_{k=1}^{N}
\operatorname{Re}
\left(
\widehat F(s_k)e^{ik\pi t/T}
\right)
\right].
\$\$
"""

# ╔═╡ b200000f-2000-4000-8000-00000000000f
...
# ╔═╡ b2000010-2000-4000-8000-000000000010
md"""
## Controlled experiment

The transform calculation is disabled initially because it performs one sparse
complex factorization per Fourier node. Set `run_fourier = true` when ready.

Changing `ensemble_size` from \$10^3\$ to \$10^6\$ does not change the transform
cost. It only rescales the expected trajectory counts.
"""

# ╔═╡ b2000011-2000-4000-8000-000000000011
begin
    run_fourier = true

    ensemble_size = 1_000_000
    number_of_switches = 4
    number_of_fourier_nodes = 16
...
# ╔═╡ b2000012-2000-4000-8000-000000000012
fourier_result = if run_fourier
...
# ╔═╡ b2000013-2000-4000-8000-000000000013
if fourier_result === nothing
    md"""
    Fourier calculation is paused. Set `run_fourier = true` in the cell above.
    """
else
    let
        figure = plot(
            xlabel="time",
            ylabel="expected number switched",
            title="$(ensemble_size) trajectories from one Fourier calculation",
            legend=:bottomright,
            size=(800, 480),
        )
...
# ╔═╡ b2000014-2000-4000-8000-000000000014
if fourier_result === nothing
    nothing
else
    (
        ...
        basin_states=length(fourier_model.states),
        boundary_kernel_size=size(fourier_model.R, 1),
        fourier_nodes=length(fourier_result.nodes),
        switches=number_of_switches,
        transform_seconds=fourier_result.elapsed_time,
        eventual_switch_mass=fourier_result.total_mass,
    )
end

# ╔═╡ b2000015-2000-4000-8000-000000000015
md"""
## What is being approximated?

The resolvent values \$K(s_k)\$ are accurate to the sparse linear-solver tolerance.
The approximation enters only through the finite Fourier series and finite time
horizon. Increasing `number_of_fourier_nodes` improves temporal resolution but
requires more shifted sparse factorizations.

This produces deterministic ensemble expectations. If stochastic population
counts are wanted afterward, the differences between adjacent CDF values can be
used as multinomial bin probabilities.
"""
