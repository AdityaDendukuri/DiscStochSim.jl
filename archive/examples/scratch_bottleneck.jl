...
rates = [1e-6, 1e-1]
prob = FSPProblem(rn, CartesianIndex(1, 0, 0), (0.0, 2e6), rates;
                  bounds=(0, Int(1e11)))

# Flux-aware run
sol_flux = solve(prob, AdaptiveFSP(ε_dt=0.1, prob_quantile=0.9, flux_tolerance=1e-9,
                                   save_interval=1000))
traj_flux = mean_trajectory(sol_flux)

# No-flux run (set flux_tolerance=0 to disable flux protection)
sol_noflux = solve(prob, AdaptiveFSP(ε_dt=0.1, prob_quantile=0.7, flux_tolerance=0.0,
                                     save_interval=1000))
