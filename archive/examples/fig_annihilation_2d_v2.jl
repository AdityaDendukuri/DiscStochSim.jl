"""
2D A+B→∅ Annihilation — Scaled Spatial Demo
============================================

8×8 fine grid solved at 4×4 coarse level (16 coarse voxels, NDIM=32).
A starts in the top-left corner, B in the bottom-right.
Slower reaction rate lets the diffusion front develop before annihilation.

Run: julia --project examples/fig_annihilation_2d_v2.jl
"""
...
const KC_x  = 2        # coarse grid rows
const KC_y  = 2        # coarse grid cols
const KF    = KF_x * KF_y   # 16 fine voxels
const KC    = KC_x * KC_y   #  4 coarse voxels
const NDIM  = 2 * KC         #  8 state dimensions

const D_A   = 1.0
const D_B   = 1.0
const K_R   = 0.3      # slower reaction — diffusion front visible before annihilation
const N_MOL = 8        # more molecules → longer-lived dynamics
const N_MAX = N_MOL
...
# Fine grid: dx_f = 0.25,  coarse: dx_c = 0.5
...
# ─── Initial condition ────────────────────────────────────────────────────────
# A in coarse voxel k=1  (top-left,     i=1,j=1)  → nA1 = N_MOL
# B in coarse voxel k=KC (bottom-right, i=KC_x,j=KC_y) → nB_{KC} = N_MOL
# State: (nA1,nB1, nA2,nB2, ..., nA_{KC},nB_{KC})
ic_tuple = ntuple(Val(NDIM)) do d
    k   = (d + 1) ÷ 2          # voxel index (1-based)
    isA = isodd(d)
    (isA && k == 1)    ? N_MOL :
    (!isA && k == KC)  ? N_MOL : 0
end
u0 = CartesianIndex(ic_tuple)
...
# --- Animated GIF ---
gif_path = joinpath(outdir, "annihilation_2d_v2.gif")
...
key_times_target = [0.0, 1.0, 3.0, 7.0]
...
Label(fig2[2, 1:5],
      ...
      "8×8 fine grid → 4×4 coarse FSP (CartesianIndex{32})   k_r=$(K_R)\n" *
      "A starts top-left, B starts bottom-right, A+B→∅ on contact",
      color=:white, fontsize=10)

png_path = joinpath(outdir, "annihilation_2d_v2_panels.png")
