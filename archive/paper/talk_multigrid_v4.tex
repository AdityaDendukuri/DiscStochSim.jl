...
% ================================================================
%  NEW: Mixed-resolution for bistable Schlögl
% ================================================================
\begin{frame}{Mixed-resolution FSP: adaptive active strip for bistable wavefront}
\begin{center}
  \includegraphics[width=0.96\textwidth,keepaspectratio]{schlogl_1d_mixed}
\end{center}
\vspace{-0.5em}
{\footnotesize
$K{=}16$ voxels, $D{=}1$, IC: left half $n_{\rm hi}{=}162$, right half $n_{\rm lo}{=}31$.
\textbf{Inactive voxels} (white overlay absent): mean-field ODE $ \dot\mu_k = f(\mu_k) + D\Delta\mu_k$; exact near fixed points.
\textbf{Active strip} (white overlay, $K_{\rm act}{\leq}3$): joint FSP tracks the transition zone exactly.\\[0.2em]
\textbf{Center}: state-space cost.
Full FSP scales as $n_{\rm eff}^K$ (red, exponential).
Mixed-resolution cost stays $\leq\!115{,}026$ states regardless of $K$ (green dashed, constant).\\[0.2em]
\textbf{Right}: why FSP is essential.
The exact single-voxel $P(n)$ is \textbf{bimodal} (mass at $n_{\rm lo}$ and $n_{\rm hi}$, white solid).
Mean-field tracks only $E[n]$, predicting a single unimodal peak at the mean (red dashed) ---
the two-state bistable structure is invisible.
}
\end{frame}


% ================================================================
\begin{frame}{2D Schl\"{o}gl: bistable front as a spatial wave}
\begin{center}
  \includegraphics[width=0.96\textwidth,keepaspectratio]{schlogl_2d}
\end{center}
\vspace{-0.4em}
{\footnotesize
\textbf{Left (4 panels):} Mean-field ODE on $40{\times}40$ grid.
IC: bottom half $n_{\rm hi}{=}162$ (red), top half $n_{\rm lo}{=}31$ (blue).
$\int f(n)\,dn > 0$: $n_{\rm hi}$ state thermodynamically favored --- front sweeps upward.
White overlay = active FSP strip ($n_{\rm lo} < \mu_k < n_{\rm hi}$).\\[0.2em]
\textbf{Right panel:} Single-voxel stationary distribution $P(n)$.
\textbf{Bimodal}: mass at $n_{\rm lo}{=}31$ and $n_{\rm hi}{=}162$, nothing near $n_{\rm unstable}{=}72$.
Mean-field sees only $E[n]$ --- it places a single mode at the current mean
and completely misses the two-state structure.
\textbf{FSP is required at the front to capture this bimodal distribution.}
Column-product: each of the $K_y$ columns gets an independent 1D FSP at the front ---
tractable regardless of $K_y$.
}
\end{frame}
