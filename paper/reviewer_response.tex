...
\begin{itemize}
  \item \textbf{Oregonator}: FP-FSP completes $t_f = 0.11$ in 37\,s (max $|\mathcal{S}|{=}1{,}063$);
        Krylov-FSP-SSA reaches only 2.7\% of the time horizon in 103\,s
        (max $|\mathcal{S}|{=}13{,}299$). The $\tau$-lock at $\tau \approx 10^{-6}$ is
  ...
  \item \textbf{Robertson ($t_f{=}10$)}: Krylov-FSP-SSA completes in 0.02\,s (101
        iterations); FP-FSP requires 877\,s. On a short horizon without bottleneck
        states the Krylov matrix exponential's large accepted steps are decisive.
        On the extended horizon $t_f{=}10^4$, however, Krylov-FSP-SSA reaches only
        $t{=}50$ in 500 iterations (0.5\% complete), while FP-FSP succeeds via
        flux-adaptive $\Delta t$ spanning six orders of magnitude.
\end{itemize}
...
concrete timings and accuracy metrics. Notably, on the short-horizon Robertson system
($t_f = 10$), Krylov-FSP-SSA completes in 0.02\,s (101 iterations) while our method
requires 877\,s --- a case where the Krylov matrix exponential's ability to take large
steps is decisive. On the Oregonator, our method completes $t_f = 0.11$ in 37\,s
while Krylov-FSP-SSA reaches only 2.7\% of the horizon in 103\,s. Section~5.7
additionally shows that even with extensive parameter tuning, Krylov-FSP-SSA cannot
achieve better than $\sim$5\% error on the bottleneck system (vs.\ $<0.1\%$ for our
method) or complete the Robertson system at $t_f = 10^4$. For other methods (Optimal
FSP, aggregation-based schemes), a full benchmark would require additional reimplementation
effort; we leave that to future work.
