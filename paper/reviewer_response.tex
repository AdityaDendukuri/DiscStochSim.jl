...
Following the reviewer's suggestion to compare against adaptive FSP methods already
cited in the paper, we first considered the exit-time FSP (ETFSP) of Kuntz et al.\
(SIAM J.\ Sci.\ Comput., 2019) as a candidate baseline. However, ETFSP targets a
fundamentally different quantity: it computes lower bounds on exit distributions
(the state distribution at the moment a trajectory first leaves the truncated domain)
and occupation measures, rather than the time-marginal distribution $p(x,t)$ at a
sequence of scheduled output times. Because ETFSP operates on a fixed truncation
without an expand--prune loop and does not produce $p(x,t)$ at intermediate times,
it is not directly applicable to the time-stepping setting we consider. Our
flux-adaptive time-step rule $\Delta t = \varepsilon_{\Delta t}/\Phi_{\mathrm{total}}$
captures the same spirit as ETFSP --- controlling the rate at which probability escapes
the truncated domain --- but as a lightweight first-order rule that is computable
from the current probability vector and integrates directly into the error analysis.

Instead, as a direct numerical comparison we implemented the Krylov-FSP-SSA
algorithm described in detail by Dinh and Sidje (Physical Biology, 2016), which
shares the same expand--evolve--prune skeleton and Krylov matrix exponential
integrator as our method and is therefore the closest algorithmic baseline. Sections~5.6
and~5.7 of the revised manuscript report a head-to-head comparison on all three
benchmark systems. Section~5.6 compares both methods under default parameters;
Section~5.7 reports the best achievable performance of Krylov-FSP-SSA after extensive
parameter tuning ($\varepsilon$, $\tau$, $r$). The results are:
\begin{itemize}
  \item \textbf{Oregonator}: FP-FSP completes $t_f = 0.11$ in 33\,s (max $|\mathcal{S}|{=}1{,}003$);
        Krylov-FSP-SSA reaches only 2.7\% of the time horizon in 74\,s
        (max $|\mathcal{S}|{=}13{,}294$). The $\tau$-lock at $\tau \approx 10^{-6}$ is
        intrinsic to the mass-conservation criterion and cannot be removed by any
        choice of $\varepsilon$.
  \item \textbf{Bottleneck}: FP-FSP achieves $\langle C \rangle = 4.97$ ($<0.1\%$ error)
        in 0.4\,s. Even with optimal tuning, the best Krylov-FSP-SSA result is
        $\langle C \rangle \approx 4.72$ ($\sim$5\% error) at comparable wall time,
        because any single $\varepsilon$ creates a fundamental conflict between
        retaining wavefront states and preventing boundary absorption.
  \item \textbf{Robertson ($t_f{=}10^4$)}: We corrected two aspects of the Krylov-FSP-SSA
        implementation since the previous version of this response. \emph{(i) Generator
        form.} Algorithm~2 of Vo \& Sidje (2016) requires an \emph{absorbing-form}
        generator in which the diagonal of $A$ includes all outgoing propensities,
        regardless of whether the destination is in the truncated set $\mathcal{S}$.
        An earlier version used a closed-form (mass-conserving) generator, which
        masked probability loss at the projection boundary and caused $\tau$ to double
        freely. \emph{(ii) Robertson figure.} The previous figure showed
        $\Delta t$ computed from snapshot-interval spacings; the revised figure shows
        the actual per-step $\delta t$ and $\tau$ from the diagnostic logs.
        With the correct absorbing-form generator, Krylov-FSP-SSA is $\tau$-locked
        at ${\approx}10^{-3}$ by the stiff $k_2 = 3\times10^6$ propensity (via FSP
        mass-loss violations and numerical overflow in the Krylov exponential at
        $B{\geq}2$ states) and reaches only $t = 228$ (2.3\% of $t_f$) in 500
        iterations (1.9\,s). FP-FSP completes $t_f = 10^4$ in 662\,472 steps
        (1\,117\,s) with per-step $\delta t$ non-monotonic and peaking at
        ${\approx}0.026$ during the equilibration phase.
\end{itemize}
For other methods (Optimal FSP, aggregation-based schemes), a full benchmark would
require more substantial reimplementation effort; we leave that to future work.
...
For the most closely related method, Krylov-FSP-SSA, we have added a direct
head-to-head comparison in Sections~5.6 and~5.7. Section~5.6 compares both methods
on the three benchmark systems (Oregonator, Bottleneck, Robertson) and reports
concrete timings and accuracy metrics. On the Oregonator, our method completes $t_f = 0.11$ in 33\,s
while Krylov-FSP-SSA reaches only 2.7\% of the horizon in 74\,s. On the Robertson
system ($t_f = 10^4$), Krylov-FSP-SSA with the correct absorbing-form generator
(Algorithm~2 of Vo \& Sidje 2016) is $\tau$-locked at ${\approx}10^{-3}$ and reaches
only $t = 228$ (2.3\%) in 500 iterations (1.9\,s); our method completes $t_f$ in
1\,117\,s. Note: an earlier version of this response reported that Krylov-FSP-SSA
completed a short-horizon Robertson problem ($t_f = 10$) in 17 steps; that result
used a closed-form (mass-conserving) generator that masked probability loss at the
boundary. With the correct absorbing-form generator, the Robertson system cannot be
completed by Krylov-FSP-SSA at any practically useful efficiency due to the
$\tau$-lock from $k_2 = 3\times10^6$ stiffness. Section~5.7 additionally shows that
even with extensive parameter tuning, Krylov-FSP-SSA cannot achieve better than
$\sim$5\% error on the bottleneck system (vs.\ $<0.1\%$ for our method). For other
methods (Optimal FSP, aggregation-based schemes), a full benchmark would require
additional reimplementation effort; we leave that to future work.
