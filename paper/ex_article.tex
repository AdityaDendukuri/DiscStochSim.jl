...
\begin{enumerate}[(i)]
\item \textbf{Total-flux rule (recommended):}
\begin{equation}
\label{eq:dt-total}
\Delta t_n = \frac{\epsilon_{\Delta t}}{\Phi_{\mathrm{total}}(J_n, t_n)}.
\end{equation}
This guarantees $\|\boldsymbol{\varepsilon}(t_{n+1})\|_1 \lesssim \epsilon_{\Delta t}$ to leading order in $\Delta t_n$, with expansion factor $\mu_n := \lambda_{\max}\,\Delta t_n = \kappa_n\,\epsilon_{\Delta t}$.
\item \textbf{Maximum-flux rule (concentrated distributions only):} Let $\Phi_{\max}(J_n,t_n) := \max_{\boldsymbol{x}\in J_n}\{p(\boldsymbol{x},t_n)\,w(\boldsymbol{x})\}$ and define the \emph{concentration ratio} $\eta_n := \Phi_{\mathrm{total}}/\Phi_{\max} \geq 1$.
\begin{equation}
\label{eq:dt-max}
\Delta t_n = \frac{\epsilon_{\Delta t}}{\Phi_{\max}(J_n, t_n)}.
\end{equation}
Since $\Phi_{\max} \leq \Phi_{\mathrm{total}}$, this step is larger than rule~(i) by factor $\eta_n$.
The error satisfies $\|\boldsymbol{\varepsilon}(t_{n+1})\|_1 \lesssim \eta_n\,\epsilon_{\Delta t}$ to leading order in $\Delta t_n$, and the expansion factor is $\mu_n = (\lambda_{\max}/\Phi_{\max})\,\epsilon_{\Delta t}$.
...
When the distribution is concentrated ($\eta_n \approx 1$, e.g., systems with conservation laws that confine probability to a narrow stoichiometric manifold), both rules yield comparable steps and the error overhead is negligible.
\textbf{Caution:} When $\eta_n \gg 1$ (spread distributions), the max-flux step $\Delta t_n^{\max} = \eta_n\,\Delta t_n^{\mathrm{tot}}$ is much larger than the total-flux step, and the leading-order bound grows as $\eta_n\,\epsilon_{\Delta t}$.
For oscillatory systems (e.g., the Oregonator), the distribution spreads widely during autocatalytic bursts---probability is distributed across $O(|\mathcal{S}|)$ states, so $\eta_n \gg 1$---and the max-flux rule takes steps that the depth-one boundary expansion cannot track, causing the wavefront to escape the active set and the solution to collapse.
The total-flux rule~(i) is free of this failure mode and is the recommended default.
\end{enumerate}
\end{corollary}
...
\begin{remark}[Maximum-flux rule as a practical speedup]
\label{rem:max-flux-speedup}
The maximum-flux rule~(ii) can serve as an efficient alternative to the total-flux
rule~(i) for stiff systems.  Because $\Delta t_n^{\max} = \eta_n\,\Delta t_n^{\mathrm{tot}}$
and the iteration count scales as $N \sim t_f/\Delta t$, using the max-flux rule with
tolerance $\epsilon_{\Delta t}^{\max}$ is equivalent in step count to the total-flux rule
with $\epsilon_{\Delta t}^{\mathrm{tot}} = \epsilon_{\Delta t}^{\max}\cdot\eta_n$.
Choosing $\epsilon_{\Delta t}^{\max} = \epsilon_{\Delta t}^{\mathrm{tot}}/\eta_n$ therefore
recovers the same iteration count while preserving the per-step error bound $\eta_n\,\epsilon_{\Delta t}^{\max} = \epsilon_{\Delta t}^{\mathrm{tot}}$.
In practice, the concentration ratio $\eta_n$ is not known in advance; however, for
the Robertson benchmark the empirical median $\eta_n \approx 107$, so setting
$\epsilon_{\Delta t}^{\max} = 0.01$ with the max-flux rule and
$\epsilon_{\Delta t}^{\mathrm{tot}} = 1.0$ with the total-flux rule produce
virtually identical step counts (41\,799 vs.\ 58\,543 for $t\in[0,100]$) and identical
accuracy ($\langle A\rangle = 6146$ in both cases).
The total-flux rule with $\epsilon_{\Delta t}=1$ (approximately one reaction per step)
is therefore the recommended universal default, as it requires no system-dependent
tuning of $\epsilon_{\Delta t}$.
\end{remark}
...
\begin{itemize}
  \item \textbf{Time step} (three options in increasing aggressiveness):
    total-flux $\Delta t = \varepsilon_{\Delta t}/\Phi_{\mathrm{total}}$ (leading-order error $\lesssim\varepsilon_{\Delta t}$, $\mu=\kappa\,\varepsilon_{\Delta t}$; \textbf{recommended});
    max-propensity cap $\Delta t = \min(\varepsilon_{\Delta t}/\Phi_{\mathrm{total}},\,1/\lambda_{\max})$ (error $\lesssim\min(\varepsilon_{\Delta t},1/\kappa)$, always $\mu\leq 1$);
    maximum-flux $\Delta t = \varepsilon_{\Delta t}/\Phi_{\max}$ (error $\lesssim\eta\,\varepsilon_{\Delta t}$, $\mu\leq 1$ when $\varepsilon_{\Delta t}\leq\Phi_{\max}/\lambda_{\max}$; valid only when $\eta_n\approx 1$, otherwise risks solution collapse for spread distributions)
...
\begin{table}[!htbp]
\centering
\caption{Recommended parameter ranges for $(\alpha,\varepsilon_{\mathrm{flux}},\varepsilon_{\Delta t})$ by system type. Values in parentheses are the parameters used in the corresponding numerical experiment of Section~\ref{sec:results}. The spatial stiffness ratio $\kappa=\lambda_{\max}/\Phi_{\mathrm{total}}$ is computable at each step.}
\label{tab:param_guidance}
\begin{tabular}{lccc}
\hline
\textbf{System type} & $\alpha$ & $\varepsilon_{\mathrm{flux}}$ & $\varepsilon_{\Delta t}$ \\
\hline
Connectivity-critical (bottleneck) & $0.7$--$0.9$ ($0.9$) & $10^{-12}$--$10^{-6}$ ($10^{-12}$) & cap: $1/\lambda_{\max}$ \\
Oscillatory / limit cycle (Oregonator) & $0.1$--$0.3$ ($0.1$) & $0$ ($0$) & $0.5$--$2$ ($1$) \\
Stiff autocatalytic (Robertson) & $0.3$--$0.5$ ($0.4$) & $10^{-9}$ ($10^{-9}$) & $0.5$--$1$ ($1$) \\
Bimodal / metastable (toggle switch) & $0.1$--$0.3$ ($0.3$) & $10^{-8}$--$10^{-6}$ ($10^{-7}$) & $0.1$--$0.5$ \\
\hline
\end{tabular}
\end{table}
...
\paragraph{Oregonator: state-space explosion.}
...
\paragraph{Bottleneck: wavefront truncation.}
...
\paragraph{Robertson: stiffness-induced $\tau$-lock.}
...
\begin{tabular}{llccc}
\hline
System & Method & Wall time & Steps / iters & Outcome \\
\hline
\multirow{2}{*}{Oregonator} & FP-FSP & 33\,s & 26 snaps & Complete; max $|\mathcal{S}|=1{,}003$ \\
 & Krylov-FSP-SSA & 74\,s & 3{,}000 & 2.7\% of $t_f$; max $|\mathcal{S}|=13{,}294$ \\
\hline
\multirow{2}{*}{Bottleneck} & FP-FSP & 1.0\,s & 1{,}000 ($\delta t{=}10$) & Complete; $\langle C \rangle=4.978$ (${<}0.1\%$ err) \\
 & Krylov-FSP-SSA & ${<}0.1$\,s & 7 ($\tau$: $100{\to}3{,}700$) & Complete; $\langle C \rangle{\approx}0$ (wavefront truncated) \\
\hline
\multirow{2}{*}{Robertson} & FP-FSP & $1{,}117$\,s & $662{,}472$ & Complete; $\langle A \rangle{=}507$, $\langle C \rangle{=}9{,}493$ \\
 & Krylov-FSP-SSA & $1.9$\,s & 500 (67\% rej.) & 2.3\% of $t_f$; incomplete \\
\hline
\end{tabular}
\end{table}
