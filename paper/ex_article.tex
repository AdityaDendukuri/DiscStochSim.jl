...
The adaptive FSP involves three coupled decisions at each step: which states to add (\emph{expansion}), how far to advance in time (\emph{time stepping}), and which states to remove (\emph{pruning}).  These are not independent heuristics---they are three facets of controlling a single quantity: the \emph{boundary outflux} $\Phi_{\mathrm{out}}$, the instantaneous rate at which probability escapes the active set.  The local error theorem (Theorem~\ref{thm:local-model-error}, Section~\ref{sec:theory}) establishes that
\begin{equation}
\label{eq:error-is-phiout}
\|\boldsymbol{\varepsilon}(t_{n+1})\|_1 \leq \int_{t_n}^{t_{n+1}}(\Phi_{\mathrm{in}}(s)+\Phi_{\mathrm{out}}(s))\,ds \leq \Phi_{\mathrm{out}}(J_n, t_n)\,\Delta t_n + O(\Delta t_n^2),
\end{equation}
where the second inequality uses $\Phi_{\mathrm{in}}(t_n)=0$ (renormalization zeroes exterior probability at each step start) and the fact that $\Phi_{\mathrm{in}}$ grows from zero during the step, making its integral second-order.  To first order in $\Delta t_n$, keeping $\Phi_{\mathrm{out}}\,\Delta t \leq \varepsilon_{\Delta t}$ is therefore the single criterion that governs accuracy.  Expansion reduces $\Phi_{\mathrm{out}}$ by moving boundary states into $J$; time stepping bounds the product $\Phi_{\mathrm{out}}\,\Delta t$; pruning removes states while limiting the increase in $\Phi_{\mathrm{out}}$.  This section develops each component in turn, keeping~\eqref{eq:error-is-phiout} as the unifying thread.
...
\begin{definition}[Boundary outflux]
\label{def:boundary-outflux}
For active set $J \subset \mathcal{X}$ and complement $J' = \mathcal{X} \setminus J$, the \textit{boundary outflux} is the rate at which probability escapes $J$:
\begin{equation}
\label{eq:boundary-outflux-def}
\Phi_{\mathrm{out}}(J,t) := \sum_{\boldsymbol{x} \in J} p(\boldsymbol{x},t) \sum_{k:\, \boldsymbol{x}+\boldsymbol{\nu}_k \notin J} \alpha_k(\boldsymbol{x}).
\end{equation}
By construction $0 \leq \Phi_{\mathrm{out}} \leq \Phi_{\mathrm{total}}$, with equality $\Phi_{\mathrm{out}} = \Phi_{\mathrm{total}}$ when $J = \{\boldsymbol{x}_0\}$ (singleton) and $\Phi_{\mathrm{out}} = 0$ when all stoichiometric neighbors of every state in $J$ lie within $J$.
\end{definition}

\begin{remark}[The central role of boundary outflux]
\label{rem:phiout-central}
The boundary outflux $\Phi_{\mathrm{out}}$ is directly computable from the absorbing-form generator diagonal: $\Phi_{\mathrm{out}} = \sum_j p_j \bigl[w(\boldsymbol{x}_j) - (-\mathbf{A}_{jj}^{\mathrm{closed}})\bigr]$, i.e., the difference between the true exit rate and the rate into states already in $J$.  The expansion strategy determines how small $\Phi_{\mathrm{out}}$ is after each expand step; the time-step rule sets $\Delta t = \varepsilon_{\Delta t}/\Phi_{\mathrm{out}}$ (or a computable upper bound thereof); and pruning is constrained to limit the increase in $\Phi_{\mathrm{out}}$.  All three decisions are unified by the single error estimate~\eqref{eq:error-is-phiout}.
\end{remark}

\begin{proposition}[Flux expansion boundary outflux bound]
\label{prop:flux-expansion-bound}
For flux expansion with threshold $\theta \in (0,1)$, where neighbor $\boldsymbol{x}+\boldsymbol{\nu}_k$ is added to $J$ iff $\alpha_k(\boldsymbol{x})/w(\boldsymbol{x}) > \theta$,
\begin{equation}
\label{eq:flux-expand-bound}
\Phi_{\mathrm{out}}(J_n, t_n) \leq \theta\,\Phi_{\mathrm{total}}(J_n, t_n).
\end{equation}
\end{proposition}

\begin{proof}
Each omitted reaction $k$ at state $\boldsymbol{x}$ satisfies $\alpha_k(\boldsymbol{x}) \leq \theta\,w(\boldsymbol{x})$, so its outflux contribution is $\alpha_k(\boldsymbol{x})\,p(\boldsymbol{x}) \leq \theta\,\Phi(\boldsymbol{x})$.  Summing over all states and all omitted reactions gives $\Phi_{\mathrm{out}} \leq \theta\,\Phi_{\mathrm{total}}$.
\end{proof}
...
These observations motivate the flux metric as a proxy for conductance-relevant activity. States with small $p(\boldsymbol{x},t)$ but large $w(\boldsymbol{x})$ have the capacity to sustain large cut flux, and protecting them is expected to reduce spectral degradation under truncation.
...
\begin{remark}[Pruning uses total flux, not boundary outflux]
\label{rem:total-vs-boundary-flux}
Remark~\ref{rem:phiout-central} identifies $\Phi_{\mathrm{out}}$ as the right quantity for time-stepping.  For \emph{pruning}, however, the per-state boundary outflux $\Phi_{\partial}(\boldsymbol{x},t):=p(\boldsymbol{x},t)\sum_{\boldsymbol{y}\in J'}\mathbf{A}_{\boldsymbol{y}\boldsymbol{x}}$ is the \emph{wrong} protection criterion.  Genuine bottleneck states send most transitions to high-probability metastable states already in $J$, so $\Phi_{\partial}(s_b) \approx 0$; the boundary-outflux criterion would \emph{fail} to protect them.  Total flux $\Phi(s_b) = p(s_b)w(s_b)$ remains large because $w(s_b)$ is large---it measures probability cycling through $s_b$ regardless of destination, which is precisely the signature of a fast-transit connector.  There is also a practical obstacle: $\Phi_{\partial}$ requires knowing $J'$, which depends on the pruning outcome, creating a circular dependency.  Hence \emph{pruning} protects states by total flux $\Phi(\boldsymbol{x}) = p(\boldsymbol{x})w(\boldsymbol{x})$, while \emph{time-stepping} is governed by the aggregate boundary outflux $\Phi_{\mathrm{out}}(J, t)$ of~\eqref{eq:boundary-outflux-def}.
\end{remark}


\subsection{Adaptive Time Stepping}
By~\eqref{eq:error-is-phiout}, the per-step error satisfies $\|\boldsymbol{\varepsilon}\|_1 \lesssim \Phi_{\mathrm{out}}(J_n,t_n)\,\Delta t_n$.  Setting $\Delta t_n = \varepsilon_{\Delta t}/\Phi_{\mathrm{out}}(J_n,t_n)$ would therefore guarantee $\|\boldsymbol{\varepsilon}\|_1 \lesssim \varepsilon_{\Delta t}$ exactly.  In practice $\Phi_{\mathrm{out}}$ depends on $J_n$, which changes at each step and may be zero after stoichiometric expansion.  Since $\Phi_{\mathrm{out}} \leq \Phi_{\mathrm{total}}$ always (Definition~\ref{def:boundary-outflux}), we use $\Phi_{\mathrm{total}}$ as the computable upper bound and choose
\begin{equation}
\label{eq:flux-timestep}
\Delta t_n = \frac{\epsilon_{\Delta t}}{\Phi_{\mathrm{total}}(\mathcal{S}_n, t_n)},
\end{equation}
which guarantees $\Phi_{\mathrm{out}}\,\Delta t_n \leq \Phi_{\mathrm{total}}\,\Delta t_n = \varepsilon_{\Delta t}$ and hence $\|\boldsymbol{\varepsilon}\|_1 \lesssim \varepsilon_{\Delta t}$ (Corollary~\ref{cor:flux-timestep}).  This bound holds for \emph{any} expansion strategy, since $\Phi_{\mathrm{out}} \leq \Phi_{\mathrm{total}}$ always.  The expansion strategy determines the tightness of the bound --- how close $\Phi_{\mathrm{out}}$ is to $\Phi_{\mathrm{total}}$ --- but not the rule~\eqref{eq:flux-timestep} itself.
...
\begin{remark}[Compressed generator vs.\ renormalization]
\label{rem:compressed-renorm}
...
Let $e_n := \|\boldsymbol{p}(t_n) - \widetilde{\boldsymbol{p}}_n\|_1$ denote the global error at step $n$, where $\widetilde{\boldsymbol{p}}_n$ is the numerical solution after pruning and renormalization at step $n$.  For $N$ time steps with $e_0 = 0$, to leading order in the pruning mass $\alpha$,
\begin{equation}
\label{eq:global-error-bound}
e_N \leq \sum_{n=0}^{N-1}(\varepsilon_n + \tau_n),
\end{equation}
where $\varepsilon_n := \int_{t_n}^{t_{n+1}} (\Phi_{\mathrm{in}}(s)+\Phi_{\mathrm{out}}(s))\,ds$ is the local model error (Theorem~\ref{thm:local-model-error}), and $\tau_n$ is the ODE error (Proposition~\ref{prop:local_time_error}).  With the total-flux step rule, $\varepsilon_n \leq \varepsilon_{\Delta t}$ to leading order (Corollary~\ref{cor:flux-timestep}).
\end{corollary}

\begin{proof}
Let $\boldsymbol{r}(t)$ be the exact CME solution on the full state space starting from $\widetilde{\boldsymbol{p}}_n$, and $\widehat{\boldsymbol{p}}(t)$ the exact solution of the compressed dynamics on $J_n$.  Since the CME semigroup is a contraction in $\ell^1$,
\[
\|\boldsymbol{p}(t_{n+1}) - \boldsymbol{r}(t_{n+1})\|_1 \leq \|\boldsymbol{p}(t_n) - \widetilde{\boldsymbol{p}}_n\|_1 = e_n.
\]
Renormalization ensures $\widetilde{\boldsymbol{p}}_{n,J'}=0$, so $\Phi_{\mathrm{in}}(t_n)=0$ exactly.  Theorem~\ref{thm:local-model-error} then gives
\[
\|\boldsymbol{r}_J(t_{n+1}) - \widehat{\boldsymbol{p}}(t_{n+1})\|_1 \leq \int_{t_n}^{t_{n+1}}(\Phi_{\mathrm{in}}(s)+\Phi_{\mathrm{out}}(s))\,ds =: \varepsilon_n.
\]
Since $\Phi_{\mathrm{out}}(s) = (\mathbf{1}^\top\mathbf{A}_{J'J})\widehat{\boldsymbol{p}}_J(s) \leq \lambda_{\max}(J_n)\|\widehat{\boldsymbol{p}}_J(s)\|_1 \leq \lambda_{\max}(J_n)$ (outflux involves only exit rates within $J_n$), and since $\Phi_{\mathrm{in}}(t_n)=0$ with $\int_{t_n}^{t_{n+1}}\Phi_{\mathrm{in}}\,ds = O(\Delta t_n^2)$ (inflow requires probability to first leave $J_n$ via $\Phi_{\mathrm{out}}$, so it is second-order), we obtain to leading order $\varepsilon_n \leq \lambda_{\max}(J_n)\,\Delta t_n$.  Let $\widetilde{\boldsymbol{p}}_{n+1}^{\mathrm{pre}}$ be the numerical solution before pruning at the end of step $n$; Proposition~\ref{prop:local_time_error} gives $\|\widehat{\boldsymbol{p}}(t_{n+1}) - \widetilde{\boldsymbol{p}}_{n+1}^{\mathrm{pre}}\|_1 \leq \tau_n$.  Pruning then zeros the exterior entries to produce $\widetilde{\boldsymbol{p}}_{n+1}$, introducing an additional perturbation of at most $\alpha$, so $\|\widehat{\boldsymbol{p}}(t_{n+1}) - \widetilde{\boldsymbol{p}}_{n+1}\|_1 \leq \tau_n + O(\alpha)$.  The triangle inequality yields $e_{n+1} \leq e_n + \varepsilon_n + \tau_n + O(\alpha)$, and summing from $n=0$ to $N-1$ gives~\eqref{eq:global-error-bound} to leading order in $\alpha$.
\end{proof}

The model error $\varepsilon_n$ measures how well the active set $J_n$ contains the evolving distribution: it equals the probability that leaks through the truncation boundary during step $n$.  The pruning and expansion decisions that determine $J_n$ are design choices that control $\Phi_{\mathrm{out}}(J_n)$ and hence $\varepsilon_n$, but they enter the error bound only through this single quantity.  In particular, the pruning mass itself does not appear: what matters is not how much probability is removed but whether the remaining active set $J_n$ retains the states through which significant probability flows.


\begin{corollary}[Adaptive time step]
\label{cor:flux-timestep}
The \emph{total-flux step rule}
\begin{equation}
\label{eq:dt-total}
\Delta t_n = \frac{\varepsilon_{\Delta t}}{\Phi_{\mathrm{total}}(J_n,t_n)}
\end{equation}
guarantees $\varepsilon_n \leq \varepsilon_{\Delta t}$ to leading order in $\Delta t_n$.
\end{corollary}

\begin{proof}
Since $\Phi_{\mathrm{in}}(t_n)=0$ (renormalization) and $\int_{t_n}^{t_{n+1}}\Phi_{\mathrm{in}}\,ds = O(\Delta t_n^2)$, Theorem~\ref{thm:local-model-error} gives $\varepsilon_n \leq \Phi_{\mathrm{out}}(t_n)\,\Delta t_n + O(\Delta t_n^2)$.  Since $\Phi_{\mathrm{out}}(J_n,t_n) \leq \Phi_{\mathrm{total}}(J_n,t_n)$ (boundary outflux is a subset of total flux), substituting $\Delta t_n = \varepsilon_{\Delta t}/\Phi_{\mathrm{total}}$ gives $\varepsilon_n \leq \varepsilon_{\Delta t} + O(\Delta t_n^2)$.  The conservative rule $\Delta t = \varepsilon_{\Delta t}/\lambda_{\max}$ (where $\lambda_{\max}(J_n) = \max_{x\in J_n} w(x)$) would give the same guarantee, but since $\Phi_{\mathrm{total}} \leq \lambda_{\max}$ the total-flux rule takes steps a factor $\lambda_{\max}/\Phi_{\mathrm{total}} \geq 1$ larger---a distribution-aware gain that grows as the probability spreads across many states.
\end{proof}


\begin{remark}[Unified parameter selection]
\label{rem:consistent-params}
The single criterion $\Phi_{\mathrm{out}}\,\Delta t \leq \varepsilon_{\Delta t}$ governs all three algorithm components (Remark~\ref{rem:phiout-central}).  Parameters follow from choosing how to split the error budget between spatial and temporal accuracy:
\begin{itemize}
  \item \textbf{Expansion strategy and depth:} choose the strategy that achieves the desired $\Phi_{\mathrm{out}}/\Phi_{\mathrm{total}}$ ratio within the state-space budget; for slow-start systems enforce a depth-one cap via the max-propensity rule (Remark~\ref{rem:max-propensity-cap}).
  \item \textbf{Time step:} $\Delta t = \varepsilon_{\Delta t}/\Phi_{\mathrm{total}}$ (Corollary~\ref{cor:flux-timestep}); for highly stiff systems cap at $1/\lambda_{\max}$ (Remark~\ref{rem:max-propensity-cap}).
  \item \textbf{Flux tolerance:} $\varepsilon_{\mathrm{flux}}$ controls which states are protected at the boundary; setting it small ensures $\Phi_{\mathrm{out}}(J_n)$ remains small, keeping $\varepsilon_n \leq \varepsilon_{\Delta t}$ (Corollary~\ref{cor:flux-timestep}).
\end{itemize}
\end{remark}

\begin{table}[!htbp]
\centering
\caption{Parameter values used in the numerical experiments of Section~\ref{sec:results}, with the ranges explored during tuning. The chosen values (in parentheses) reflect the reasoning described in Remark~\ref{rem:param-guidance}.}
\label{tab:param_guidance}
\begin{tabular}{lcc}
\hline
\textbf{System type} & $\alpha$ & $\varepsilon_{\Delta t}$ \\
\hline
Connectivity-critical (bottleneck) & $0.7$--$0.9$ ($0.9$) & cap: $1/\lambda_{\max}$ \\
Oscillatory / limit cycle (Oregonator) & $0.1$--$0.3$ ($0.1$) & $0.5$--$2$ ($1$) \\
Stiff autocatalytic (Robertson) & $0.3$--$0.5$ ($0.4$) & $0.5$--$1$ ($1$) \\
Bimodal / metastable (toggle switch) & $0.1$--$0.3$ ($0.3$) & $0.1$--$0.5$ \\
\hline
\end{tabular}
\end{table}

\begin{remark}[Parameter choices for numerical experiments]
\label{rem:param-guidance}
Table~\ref{tab:param_guidance} records the parameter values used in Section~\ref{sec:results} and the ranges explored during tuning.  The choices are motivated as follows.

\textbf{$\varepsilon_{\Delta t}$:} For well-mixed systems with $\kappa \approx 1$ (Oregonator, Robertson at steady state), $\Phi_{\mathrm{total}} \approx \lambda_{\max}$ and $\varepsilon_{\Delta t} = 1$ keeps per-step error at most~$1$; we verified this produced accurate moments.  For long integrations where errors accumulate (Corollary~\ref{cor:global-error}), smaller values ($0.01$--$0.1$) may be needed.  For the bottleneck system the step is dominated by the max-propensity cap $1/\lambda_{\max} = 1/k_2$, so $\varepsilon_{\Delta t}$ is effectively inactive and any value above the cap is equivalent.

\textbf{$\alpha$:} For the bottleneck system, setting $\alpha = 0.9$ aggressively prunes the probability tail while the flux criterion separately protects the connector state $B{=}1$; reducing $\alpha$ gave no accuracy benefit but increased state-space size.  For the Oregonator and toggle switch, moderate values ($0.1$--$0.3$) were sufficient to track the oscillatory or bimodal distribution without over-pruning.

\textbf{$\varepsilon_{\mathrm{flux}}$:} For systems without connectivity bottlenecks (Oregonator, Robertson, toggle switch) we set $\varepsilon_{\mathrm{flux}} = 0$, since the active set is simply connected and flux-based retention would only add unnecessary boundary states.  For the bottleneck system, $\varepsilon_{\mathrm{flux}} = 10^{-12}$ was chosen to match the expected relative flux of the connector state $p(B{=}1)\,w(B{=}1)/\Phi_{\mathrm{total}} \approx 10^{-12}$; any value in $10^{-12}$--$10^{-6}$ gave identical results.
\end{remark}
...
With only $R=2$ reactions, stoichiometric expansion adds at most two neighbors per active state per step, so state-space growth is naturally bounded.  We use \emph{stoichiometric expansion} throughout; the depth-one cap $\Delta t \leq 1/\lambda_{\max} = 1/k_2 = 10$\,s is applied explicitly because $\Phi_{\mathrm{total}} \to 0$ at $t=0$ (Remark~\ref{rem:max-propensity-cap}), keeping the expansion depth well-matched to the probability wavefront.
...
At the steady-state initial condition, the total flux is $\Phi_{\mathrm{total}} \approx 158{,}000\,\text{s}^{-1}$ and the maximum exit rate is $\lambda_{\max} = w(N_X^*, N_Y^*, N_Z^*) = 158{,}000\,\text{s}^{-1}$, giving spatial stiffness ratio $\kappa = \lambda_{\max}/\Phi_{\mathrm{total}} = 1$.
Since $\kappa = 1$ and $\varepsilon_{\Delta t} = 1$, the expansion factor $\mu = \kappa\,\varepsilon_{\Delta t} = 1$, so depth $d=1$ suffices and we set \texttt{expansion\_depth = 1}.
...
\begin{remark}[Depth-aware cap for slow-start systems]
\label{rem:max-propensity-cap}
The condition $\mu_n \le d$ is automatically satisfied whenever $\kappa_n\,\varepsilon_{\Delta t} \le d$. For \emph{slow-start} systems where $\Phi_{\mathrm{total}} \to 0$ at $t=0$ (e.g.\ the Bottleneck system with $k_1 \ll k_2$), the flux step diverges and depth-$d$ containment must be enforced explicitly. The implemented time step is
\begin{equation}
\label{eq:dt-cap}
\Delta t_n = \min\!\left(\frac{\varepsilon_{\Delta t}}{\Phi_{\mathrm{total}}(J_n,t_n)},\;\frac{d}{\lambda_{\mathrm{downstream}}},\;t_f - t_n\right),
\end{equation}
where $\lambda_{\mathrm{downstream}}$ is the exit rate of the fast downstream states first reached by expansion. In the implementation, the user supplies $\Delta t_{\max} = 1/\lambda_{\mathrm{downstream}}$ and the code applies the cap $d\cdot\Delta t_{\max}$, so increasing expansion depth automatically relaxes the cap proportionally without requiring any parameter change.
\end{remark}
...
\begin{table}[!htbp]
\centering
\caption{FP-FSP vs.\ Krylov-FSP-SSA summary. Krylov parameters: Oregonator $(\varepsilon{=}10^{-2},\,\tau_0{=}10^{-6},\,r{=}1)$; Bottleneck $(\varepsilon{=}10^{-2},\,\tau_0{=}100,\,r{=}1)$; Robertson $(\varepsilon{=}10^{-2},\,\tau_0{=}0.1,\,r{=}1,\,\text{absorbing gen.})$. FP-FSP parameters: Oregonator $(\varepsilon_{\Delta t}{=}1,\,\alpha{=}0.1,\,\varepsilon_{\mathrm{flux}}{=}1)$; Bottleneck $(\delta t{=}10,\,\alpha{=}0.9,\,\varepsilon_{\mathrm{flux}}{=}10^{-6})$; Robertson $(\varepsilon_{\Delta t}{=}0.01,\,\alpha{=}0.4,\,\varepsilon_{\mathrm{flux}}{=}10^{-9})$.}
\label{tab:comparison_summary}
\begin{tabular}{llcc}
\hline
System & Method & Wall time & Steps / iters \\
\hline
\multirow{2}{*}{Oregonator} & FP-FSP & 36\,s & 26 snaps \\
 & Krylov-FSP-SSA & 74\,s & 3{,}000 (2.7\% of $t_f$) \\
\hline
\multirow{2}{*}{Bottleneck} & FP-FSP & 1.0\,s & 1{,}000 ($\delta t{=}10$) \\
 & Krylov-FSP-SSA & ${<}0.1$\,s & 7 ($\tau$: $100{\to}3{,}700$) \\
\hline
\multirow{2}{*}{Robertson} & FP-FSP & $1{,}117$\,s & $662{,}472$ \\
 & Krylov-FSP-SSA & $1.9$\,s & 500 (67\% rej.; 2.3\% of $t_f$) \\
\hline
\end{tabular}
\end{table}
