...
\paragraph{Robertson.}
Table~\ref{tab:comparison_robertson} and Figure~\ref{fig:robertson_comparison} compare the time-stepping behaviour on the Robertson system ($A \to B$, $2B \to C+B$, $C+B \to A+C$; $k_1 = 0.04$, $k_2 = 3\times10^6$, $k_3 = 1$, $A_0 = 10{,}000$, $B_0 = C_0 = 0$) over $[0, 10^4]$. This system is deterministically stiff ($k_2 \gg k_1, k_3$), but the stochastic initial condition ($B_0 = 0$) shapes how each method experiences that stiffness.

For FP-FSP, the flux-adaptive time step $\delta t = \varepsilon_{\Delta t}/\Phi_{\mathrm{total}}$ grows as $A$ is slowly consumed. At $t = 0$, $\Phi_{\mathrm{total}} \approx k_1 A_0 = 400$, giving $\delta t \approx 2.5\times10^{-3}$. As $A$ converts to $C$ over time, $\Phi_{\mathrm{total}} \approx k_1 A(t)$ decreases monotonically and $\delta t$ grows correspondingly, reaching ${\sim}10^2$ near $t_f = 10^4$ (Figure~\ref{fig:robertson_comparison}a). FP-FSP completes the full horizon with a compact state space maintained by flux-based pruning.

For Krylov-FSP-SSA, $\tau$ doubles after every accepted step with zero FSP rejections. Starting from $(A_0, 0, 0)$, the quasi-steady state $B_{\mathrm{ss}} \approx \sqrt{k_1 A / k_2} \approx 0.01 \ll 1$ means the FSP mass is conserved at any $\tau$: the system barely moves in state space and no probability leaves the projection boundary. Accordingly, $\tau$ grows exponentially---0.1, 0.2, 0.4, \ldots---completing $t_f = 10^4$ in 17 steps (${\approx}0.01$\,s). The matrix exponential resolves the fast $k_2 B^2$ timescale internally without exposing it as a step-size constraint.

\begin{table}[!htbp]
\centering
\caption{Robertson comparison ($t_f = 10^4$): FP-FSP (flux-adaptive $\delta t$, growing from ${\approx}2.5\times10^{-3}$ to ${\sim}10^2$) vs.\ Krylov-FSP-SSA (Algorithm~2 of~\cite{VoSidje2016}, $\varepsilon=10^{-2}$, $\tau_{\mathrm{init}}=0.1$, $r=1$). Both methods complete $t_f$.}
...
\hline
Wall time (s) & --- & ${<}0.1$ \\
Steps / Iterations & --- & 17 ($\tau$: $0.1 \to 3{,}447$) \\
Rejections & --- & 0 \\
Time reached & $10^4$ (complete) & $10^4$ (complete) \\
$\delta t$ / $\tau$ range & $2.5\times10^{-3} \to {\sim}10^2$ & $0.1 \to 3{,}447$ \\
\hline
\end{tabular}
\end{table}
...
\begin{figure}[!htbp]
    ...
    \caption{Robertson system ($t_f = 10^4$): time step vs.\ iteration (log scale). (a) FP-FSP $\delta t$: grows monotonically from ${\approx}2.5\times10^{-3}$ (set by $\Phi_{\mathrm{total}} \approx k_1 A_0 = 400$ at $t{=}0$) to ${\sim}10^2$ near $t_f$ as $A$ is consumed and $\Phi_{\mathrm{total}}$ decreases; the bimolecular states ($B{\geq}2$) in the projection carry negligible probability and therefore negligible flux weight. (b) Krylov-FSP-SSA $\tau$: doubles after each accepted step from $\tau_{\mathrm{init}} = 0.1$ to $\tau \approx 3{,}447$, with zero FSP rejections, completing $t_f = 10^4$ in 17 steps; starting from $B_0{=}0$, the quasi-steady state $B_{\mathrm{ss}} \approx 0.01$ means FSP mass is conserved at any $\tau$.}
    \label{fig:robertson_comparison}
\end{figure}

\paragraph{Summary.}
The comparison reveals clearly complementary strengths rooted in different algorithmic choices about step-size control and pruning. On the oscillatory Oregonator, flux-adaptive time stepping provides a decisive advantage: FP-FSP scales $\delta t$ to the instantaneous activity level and maintains a compact state space via flux-based pruning, while Krylov-FSP-SSA is locked at $\tau \approx 10^{-6}$ by mass-conservation failures and cannot complete $t_f = 0.11$. On the bottleneck system, FP-FSP with a fixed step $\delta t = 1/k_2 = 10$\,s keeps the depth-1 expansion pace-matched with the probability wavefront; Krylov-FSP-SSA's exponentially growing $\tau$ produces a 14-state projection that underestimates $\langle C \rangle$ by 40$\times$, with no FSP rejection signal because the missing-row truncation does not reduce $\sum w$ within the truncated system. On Robertson, both methods complete $t_f = 10^4$: FP-FSP's probability-weighted flux $\Phi_{\mathrm{total}} \approx k_1 A(t)$ decreases as $A$ is consumed, allowing $\delta t$ to grow smoothly from ${\approx}2.5\times10^{-3}$ to ${\sim}10^2$; Krylov-FSP-SSA exploits the quasi-steady $B_{\mathrm{ss}} \approx 0.01$ to double $\tau$ freely, finishing in just 17 steps.
...
\textbf{Robertson ($t_f = 10^4$).} Both methods complete $t_f = 10^4$. Krylov-FSP-SSA finishes in 17 steps (${\approx}0.01$\,s), with $\tau$ doubling from 0.1 to ${\approx}3{,}447$ and zero rejections, because starting from $B_0 = 0$, the stiff $k_2 B^2$ dynamics are not triggered in the FSP sense. FP-FSP's probability-weighted flux $\Phi_{\mathrm{total}} \approx k_1 A(t)$ decreases as $A$ is consumed, allowing $\delta t$ to grow from ${\approx}2.5\times10^{-3}$ to ${\sim}10^2$ over the simulation. The step count is therefore much larger than for Krylov-FSP-SSA, but each step is cheap due to the compact, flux-pruned state space. On Robertson, Krylov-FSP-SSA is the more efficient method.
...
Robertson $t_f{=}10^4$ & ---\,s, many steps & ${<}0.1$\,s, 17 iters & --- & Krylov-FSP wins \\
\hline
\end{tabular}
\end{table}

The key findings reveal qualitatively distinct failure modes. On the Oregonator, the Krylov $\tau$-lock is intrinsic to the mass-conservation criterion and cannot be resolved by any parameter choice: large propensities ($k_3 X Z \sim 10^5$) force $\tau \lesssim 10^{-6}$ regardless of $\varepsilon$. On the bottleneck system, the exponential growth of $\tau$ creates a fundamental conflict with depth-$r$ expansion: keeping the projection pace-matched with the wavefront requires $r \gtrsim k_2 \tau$, but $\tau$ grows without bound; the best achievable accuracy is $\sim$5\% error regardless of $(\varepsilon, \tau, r)$. On Robertson, both methods succeed, but with qualitatively different step-size dynamics: FP-FSP's probability-weighted flux control allows $\delta t$ to grow as $A$ is consumed, while Krylov-FSP-SSA's matrix exponential absorbs the stiff $k_2 B^2$ timescale internally, reaching $t_f = 10^4$ in 17 steps. The complementary picture is clear: flux-based methods excel on problems with oscillatory dynamics or connectivity bottlenecks where large propensities at high-probability states or conduit states require careful step-size and pruning control; Krylov methods excel on stiff problems where the stiff timescale is not directly relevant to the dominant distribution, and the matrix exponential can step over it without mass loss.
