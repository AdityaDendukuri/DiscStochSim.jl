...
\end{equation}
limits the leading-order fraction of probability mass undergoing a transition per step to $O(\varepsilon_{\Delta t})$, and automatically shortens steps during fast or stiff phases while lengthening them during slow transients.
\end{proposition}
\begin{proof}
The exact CME semigroup satisfies $\boldsymbol{p}(t_n+\Delta t_n) = e^{\widetilde{\mathbf{A}}_{JJ}\Delta t_n}\widetilde{\boldsymbol{p}}_n$; expanding to first order gives $\boldsymbol{p}(t_n+\Delta t_n) - \widetilde{\boldsymbol{p}}_n = \widetilde{\mathbf{A}}_{JJ}\widetilde{\boldsymbol{p}}_n\,\Delta t_n + O(\Delta t_n^2)$.  Since $\widetilde{\mathbf{A}}_{JJ}$ has zero column sums, $\widetilde{\mathbf{A}}_{JJ}\widetilde{\boldsymbol{p}}_n$ sums to zero; its positive and negative parts are equal, each summing to $\Phi_{\mathrm{total}}(J_n,t_n)$.  Therefore
\[
\|\boldsymbol{p}(t_n+\Delta t_n) - \widetilde{\boldsymbol{p}}_n\|_1 = 2\,\Phi_{\mathrm{total}}(J_n,t_n)\,\Delta t_n + O(\Delta t_n^2),
\]
and the choice $\Delta t_n = \varepsilon_{\Delta t}/\Phi_{\mathrm{total}}$ makes this $O(\varepsilon_{\Delta t})$.  The adaptive property follows because $\Phi_{\mathrm{total}} = \mathbb{E}_p[w(\boldsymbol{x})]$ tracks the active timescale.
\begin{itemize}
\item \emph{Slow transient or quiescent phase.} Probability is concentrated in low-propensity states, so $\Phi_{\mathrm{total}}$ is small and $\Delta t_n$ is large.
\item \emph{Fast or stiff phase.} Probability shifts into high-propensity states, $\Phi_{\mathrm{total}}$ grows, and $\Delta t_n$ shrinks automatically without explicit phase detection.
\end{itemize}
\end{proof}
