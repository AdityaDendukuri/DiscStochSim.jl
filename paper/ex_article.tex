...
\end{equation}
For $d=1$, using $\mathrm{P}(\mathrm{Poisson}(\mu_n)\geq 1) = 1-e^{-\mu_n} \leq \mu_n$, the bound reduces to $\mu_n^2$.
Depth $d=1$ is therefore sufficient whenever $\mu_n \leq 1$; under the total-flux rule~\eqref{eq:dt-total} this condition reads $\kappa_n\,\varepsilon_{\Delta t} \leq 1$, where $\kappa_n := \lambda_{\max}(J_n,t_n)/\Phi_{\mathrm{total}}(J_n,t_n)$ is the spatial stiffness ratio.
\end{corollary}
...
\begin{remark}[Max-propensity cap for depth-one guarantee]
\label{rem:max-propensity-cap}
Under the total-flux rule, $\mu_n = \kappa_n\,\epsilon_{\Delta t}$ exceeds one when $\kappa_n > 1/\epsilon_{\Delta t}$ (high spatial stiffness), which would require expansion depth $d > 1$.  For highly stiff systems, depth-one sufficiency can be enforced unconditionally by applying the \emph{max-propensity cap}:
\begin{equation}
\label{eq:dt-cap}
...
When the cap activates ($\kappa_n\,\epsilon_{\Delta t}>1$), the step is $\Delta t_n = 1/\lambda_{\max}$, giving local error $\|\boldsymbol{\varepsilon}(t_{n+1})\|_1 \lesssim 1/\kappa_n \leq \epsilon_{\Delta t}$ --- actually smaller than the prescribed tolerance while expansion depth remains one.
The cap therefore trades more frequent steps for cheaper per-step cost: depth-$d$ expansion requires $O(d\,|\mathcal{S}|\,R)$ work, so reducing $d$ from $\lceil\kappa\,\varepsilon_{\Delta t}\rceil$ to $1$ can yield a net simulation speedup when $\kappa$ is large.
\end{remark}
