...
The adaptive FSP involves three coupled decisions at each step: which states to add (\emph{expansion}), how far to advance in time (\emph{time stepping}), and which states to remove (\emph{pruning}).  These are not independent heuristics---they are three facets of controlling a single quantity: the \emph{boundary outflux} $\Phi_{\mathrm{out}}$, the instantaneous rate at which probability escapes the active set.  The local error theorem (Theorem~\ref{thm:local-model-error}, Section~\ref{sec:theory}) establishes that after renormalization $\Phi_{\mathrm{in}}(t_n)=0$ and
\begin{equation}
\label{eq:error-is-phiout}
\|\boldsymbol{\varepsilon}(t_{n+1})\|_1 \lesssim \Phi_{\mathrm{out}}(J_n, t_n)\,\Delta t_n,
\end{equation}
so keeping $\Phi_{\mathrm{out}}\,\Delta t \leq \varepsilon_{\Delta t}$ is the single criterion that governs accuracy.  Expansion reduces $\Phi_{\mathrm{out}}$ by moving boundary states into $J$; time stepping bounds the product $\Phi_{\mathrm{out}}\,\Delta t$; pruning removes states while limiting the increase in $\Phi_{\mathrm{out}}$.  This section develops each component in turn, keeping~\eqref{eq:error-is-phiout} as the unifying thread.
...
\end{equation}
where $\varepsilon_n := \int_{t_n}^{t_{n+1}} (\Phi_{\mathrm{in}}(s)+\Phi_{\mathrm{out}}(s))\,ds \leq \lambda_{\max}(J_n)\,\Delta t_n$ is the local model error (Theorem~\ref{thm:local-model-error}), and $\tau_n$ is the ODE error (Proposition~\ref{prop:local_time_error}).
\end{corollary}
...
\[
\|\boldsymbol{r}_J(t_{n+1}) - \widehat{\boldsymbol{p}}(t_{n+1})\|_1 \leq \int_{t_n}^{t_{n+1}}(\Phi_{\mathrm{in}}(s)+\Phi_{\mathrm{out}}(s))\,ds =: \varepsilon_n \leq \lambda_{\max}(J_n)\,\Delta t_n,
\]
using $\Phi_{\mathrm{in}}+\Phi_{\mathrm{out}} \leq \lambda_{\max}\|\boldsymbol{r}(s)\|_1 = \lambda_{\max}$ pointwise.  Let $\widetilde{\boldsymbol{p}}_{n+1}^{\mathrm{pre}}$ be the numerical solution before pruning at the end of step $n$; Proposition~\ref{prop:local_time_error} gives $\|\widehat{\boldsymbol{p}}(t_{n+1}) - \widetilde{\boldsymbol{p}}_{n+1}^{\mathrm{pre}}\|_1 \leq \tau_n$.  Pruning then zeros the exterior entries to produce $\widetilde{\boldsymbol{p}}_{n+1}$, introducing an additional perturbation of at most $\alpha$, so $\|\widehat{\boldsymbol{p}}(t_{n+1}) - \widetilde{\boldsymbol{p}}_{n+1}\|_1 \leq \tau_n + O(\alpha)$.  The triangle inequality yields $e_{n+1} \leq e_n + \varepsilon_n + \tau_n + O(\alpha)$, and summing from $n=0$ to $N-1$ gives~\eqref{eq:global-error-bound} to leading order in $\alpha$.
\end{proof}
...
Let $\kappa_n := \lambda_{\max}(J_n)/\Phi_{\mathrm{total}}(J_n,t_n) \geq 1$ be the \emph{spatial stiffness ratio}, where $\lambda_{\max}(J_n) := \max_{\boldsymbol{x}\in J_n} w(\boldsymbol{x})$.
The \emph{total-flux step rule}
\begin{equation}
\label{eq:dt-total}
\Delta t_n = \frac{\varepsilon_{\Delta t}}{\Phi_{\mathrm{total}}(J_n,t_n)}
\end{equation}
guarantees the exact bound
\begin{equation}
\label{eq:model-error-bound}
\varepsilon_n \leq \kappa_n\,\varepsilon_{\Delta t}.
\end{equation}
For $\kappa_n = O(1)$---which holds whenever the distribution is not strongly concentrated on the single highest-rate state---the per-step model error is $O(\varepsilon_{\Delta t})$.
\end{corollary}

\begin{proof}
From the global error proof, $\varepsilon_n \leq \lambda_{\max}(J_n)\,\Delta t_n$ (since $\Phi_{\mathrm{in}}+\Phi_{\mathrm{out}} \leq \lambda_{\max}$ pointwise, as both are bounded by $\lambda_{\max}\|\boldsymbol{r}(s)\|_1 = \lambda_{\max}$).  Substituting $\Delta t_n = \varepsilon_{\Delta t}/\Phi_{\mathrm{total}}$:
\[
\varepsilon_n \leq \lambda_{\max}(J_n)\cdot\frac{\varepsilon_{\Delta t}}{\Phi_{\mathrm{total}}(J_n,t_n)} = \kappa_n\,\varepsilon_{\Delta t}.
\]
Using $\Phi_{\mathrm{total}}$ rather than $\lambda_{\max}$ in the denominator gives a larger, distribution-aware step: $\Phi_{\mathrm{total}} = \sum_x p(x)w(x)$ is the probability-weighted average of exit rates, whereas $\lambda_{\max}$ is the worst-case single-state rate.  When the distribution is spread across many states, $\Phi_{\mathrm{total}} \ll \lambda_{\max}$ and the rule $\Delta t_n = \varepsilon_{\Delta t}/\Phi_{\mathrm{total}}$ takes steps far larger than $\varepsilon_{\Delta t}/\lambda_{\max}$ would permit, while the bound $\varepsilon_n \leq \kappa_n\varepsilon_{\Delta t}$ remains valid.
\end{proof}
...
\begin{itemize}
  \item \textbf{Expansion strategy and depth:} choose the strategy that achieves the desired $\Phi_{\mathrm{out}}/\Phi_{\mathrm{total}}$ ratio within the state-space budget; use $d=1$ when $\kappa\,\varepsilon_{\Delta t}\leq 1$, or enforce it via the max-propensity cap (Remark~\ref{rem:max-propensity-cap}) for slow-start systems.
  \item \textbf{Time step:} $\Delta t = \varepsilon_{\Delta t}/\Phi_{\mathrm{total}}$; for highly stiff systems cap at $d/\lambda_{\max}$ (Remark~\ref{rem:max-propensity-cap}).
  \item \textbf{Flux tolerance:} $\varepsilon_{\mathrm{flux}}$ controls which states are protected at the boundary; setting it small ensures $\Phi_{\mathrm{out}}(J_n)$ remains small, keeping $\varepsilon_n$ within the budget set by Corollary~\ref{cor:flux-timestep}.
\end{itemize}
\end{remark}

\begin{table}[!htbp]
\centering
\caption{Recommended ranges for $\alpha$ and $\varepsilon_{\Delta t}$ by system type. Values in parentheses are the parameters used in the corresponding numerical experiment of Section~\ref{sec:results}. For flux tolerance guidance see Remark~\ref{rem:param-guidance}.}
...
\begin{remark}[Practical parameter selection]
\label{rem:param-guidance}
Table~\ref{tab:param_guidance} summarises concrete recommendations complementing the theoretical framework of Remark~\ref{rem:consistent-params}.
Three rules of thumb emerge.

\textbf{$\varepsilon_{\Delta t}$:} Choose $\varepsilon_{\Delta t}\leq 1/\kappa$ so that depth-one expansion suffices; for $\kappa\approx 1$ this permits $\varepsilon_{\Delta t}$ up to~1.
Reduce $\varepsilon_{\Delta t}$ (to $0.01$--$0.1$) when a long integration horizon demands high accuracy, since local errors accumulate additively (Corollary~\ref{cor:global-error}).

\textbf{$\alpha$:} For systems with connectivity bottlenecks, $\alpha$ can be set aggressively ($0.7$--$0.9$) because the flux criterion rescues any misclassified connector states; the quantile filter then removes only the genuinely irrelevant probability tail.
Without bottlenecks, moderate values ($0.1$--$0.3$) avoid over-pruning.

\textbf{$\varepsilon_{\mathrm{flux}}$:} Set to zero for oscillatory or well-mixed systems where the active set is simply connected and no low-probability connector states need protection (a positive value would retain unnecessary states near the expanding boundary).
